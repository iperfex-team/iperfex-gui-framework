...
function obtenerDetallesRPMS()
{
    ...
    foreach ($packageClass as $sTag => $packageLists) {
        ...
        if ($sTag == 'Kernel') {
            ...
            // El paquete iperfex debe ir primero
            if (isset($packageVersions['iperfex']))
                $result[$sTag][] = $packageVersions['iperfex'];
            foreach ($packageVersions as $packageName => $fields) {
                if (substr($packageName, 0, 8) == 'iperfex-')
                    $result[$sTag][] = $fields;
            }
        } else {
...
{
    global $arrConf;
    include_once "libs/iperfexACL.class.php";
    ...
    if($uid===FALSE)
        ...
        if($val === TRUE){
            ...
            if($status){
                ...
                $arrResult['msg'] = _tr("Iperfex password has been changed.");
                $_SESSION['iperfex_pass'] = md5($new_pass);
                $_SESSION['iperfex_pass2'] = $new_pass;
            }else{
                $arrResult['msg'] = _tr("Impossible to change your Iperfex password.")." ".$pORG->errMsg;
            }
        }else{
            $arrResult['msg'] = _tr("Impossible to change your Iperfex password. User does not exist or password is wrong");
        }
    }
    return $arrResult;
}
...
function searchModulesByName()
{
    ...
    include_once "apps/group_permission/libs/iperfexGroupPermission.class.php";
    $json = new Services_JSON();

    $pGroupPermission = new iperfexGroupPermission();
    ...
    $pACL = new iperfexACL($arrConf['iperfex_dsn']['iperfex']);
    $pMenu = new iperfexMenu($arrConf['iperfex_dsn']['iperfex']);
    ...
    $org_access=null;
    if(!$pACL->isUserSuperAdmin($_SESSION['iperfex_user'])){
        $org_access='yes';
    }
        
    $arrSessionPermissions = $pMenu->filterAuthorizedMenus($pACL->getIdUser($_SESSION['iperfex_user']),$administrative);
...
function putMenuAsBookmark($menu)
{
    ...
    if($menu != ""){
        ...
        if($uid!==FALSE){
            ...
            $org_access=(!$pACL->isUserSuperAdmin($_SESSION['iperfex_user']))?'yes':NULL;
            ...
            if($arr_result1 !== FALSE){
                ...
                if($exist){
                    ...
                    if(!$r){
                        ...
                        $arrResult['msg'] = _tr("Bookmark cannot be removed. Please try again or contact with your iperfex administrator and notify the next error: ").$pdbACL->errMsg;
                        return $arrResult;
                    }else{
                        $pdbACL->commit();
                        $arrResult['status'] = TRUE;
                ...
                if(count($arr_result1) > 4){
                    ...
                    if(!$r){
                        ...
                        $arrResult['msg'] = _tr("Bookmark cannot be added. Please try again or contact with your iperfex administrator and notify the next error: ").$pdbACL->errMsg;
                    }else{
                        $pdbACL->commit();
                        $arrResult['status'] = TRUE;
                        $arrResult['data'] = array("action" => "add", "menu" => _tr($resource[0][1]), "idmenu" => $menu,  "menu_session" => $menu );
        ...
        if($uid!==FALSE){
            ...
            if($exist){
                ...
                if(!$r){
                    $pdbACL->rollBack();
                    $arrResult['status'] = FALSE;
                    $arrResult['msg'] = _tr("Request cannot be completed. Please try again or contact with your iperfex administrator and notify the next error: ").$pdbACL->errMsg;
                    return $arrResult;
                }else{
                    $pdbACL->commit();
                    $arrResult['status'] = TRUE;
...
{
    $query = 'SELECT property_name,property_val from iperfex_chat_config';
    ...
    if ($result===false) {
        ...
        //mostramos un error
        $error = 'Error to obtain iperfex chat configurations';
        return false;
    }
    ...
    $http['prefix'] = (isset($http['prefix']) && !empty($http['prefix']))
        ? '/'.$http['prefix'] : '';
    $chat_conf['iperfex_chat_server'] = $_SERVER['SERVER_NAME'];
    $chat_conf['ws_servers'] = "{$type_connection}://{$chat_conf['iperfex_chat_server']}:{$puerto}{$http['prefix']}/ws";
    return $chat_conf;
}
...
function getDataProfile($pDB, &$ERROR){
    $arrCredentials=getUserCredentials($_SESSION['iperfex_user']);
...
function getNewListIperfexAccounts($searchFilter, &$errmsg)
{
    global $arrConf;
    ...
    //1) obtenemos los parametros generales de configuracion para asterisk websocket y el cliente de chat de iperfex
    ...
    //   el ws_server
    $dominio = $chatConfig['iperfex_chat_server'];
            ...
            */
            if ($value['extension'] != '' && isset($value['extension'])) {
                ...
                if ($value['id'] != $arrCredentials['idUser']) {   
                    ...
                    $arrContacts[$index_st][$key]['uri']="{$value['iperfexweb_device']}@$dominio";
                    $arrContacts[$index_st][$key]['alias']="{$value['alias']}@$dominio";
                }else{
                    $arrContacts['my_info']['uri']="{$value['iperfexweb_device']}@$dominio";
                    $arrContacts['my_info']['ws_servers']=$chatConfig['ws_servers'];
                    $arrContacts['my_info']['password']=$_SESSION['iperfex_pass2'];
                    $arrContacts['my_info']['display_name']=$value['name'];
                    $arrContacts['my_info']['iperfexuser_username']=$value['username'];
                    $arrContacts['my_info']['iperfexuser_exten']=$value['extension'];
                    $arrContacts['my_info']['iperfexuser_faxexten']=$value['fax_extension'];
...
function getChatContactsStatus($searchFilter)
{
    ...
    $dummy = NULL;    
    $newListContacts=getNewListIperfexAccounts($searchFilter, $dummy);
