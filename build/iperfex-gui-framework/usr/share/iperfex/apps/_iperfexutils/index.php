...
*/
require_once "libs/iperfexJSON.class.php";

function _moduleContent(&$smarty, $module_name)
{
    require_once "apps/$module_name/libs/iperfexutils.lib.php";
...
function handleJSON_changePasswordIperfex($smarty, $module_name)
{
...
function handleJSON_showAboutAs($smarty, $module_name)
{
    global $arrConf;
    $jsonObject   = new PaloSantoJSON();
    $about_us_content=_tr('About Iperfex Content');
    $html="<table border='0' cellspacing='0' cellpadding='2' width='100%'>".
            "<tr class='tabForm' >".
                "<td class='tabForm' align='center'>".
                    "$about_us_content<br />".
                    "<a href='http://www.iperfex.com' target='_blank'>www.iperfex.org</a>".
                "</td>".
            "</tr>".
          "</table>";
    ...
    $response['html']  = $html;
    $response['title'] = _tr('About Iperfex')." ".$arrConf['iperfex_version'];

    if($arrConf['mainTheme']=="iperfexwave" || $arrConf['mainTheme']=="iperfexneo")
        $response['title'] = _tr('About Iperfex2');
...
function handleJSON_getIperfexAccounts($smarty, $module_name)
{
    Header('Content-Type: application/json');
    $jsonObject = new PaloSantoJSON();
    ...
    $errmsg = NULL;
    $arrContacts = getNewListIperfexAccounts($searchFilter, $errmsg);
...
function handleJSON_getSIPParameters($smarty, $module_name)
{
     ...
     * iperfexuser_username display_name password  */
    $paramSIP += array(
        'iperfexuser_username'  =>  str_replace('IM_', 'IM@', $accountInfo['iperfexweb_device']),
        'display_name'      =>  $accountInfo['name'],
        'password'          =>  $_SESSION['iperfex_pass2']
    );
...
function handleJSON_getSIPRoster($smarty, $module_name)
{
    ...
    foreach ($accountList as $tupla) if ($tupla['id'] != $arrCredentials['idUser']) {
        $result[] = array(
            ...
            'uri'           =>  $tupla['iperfexweb_device'].'@'.$paramSIP['iperfex_chat_server'],
...
function handleJSON_getUserProfile($smarty, $module_name){
    ...
    $arrFormFilter = createProfileForm($langIperfex);
    $oFilterForm = new iperfexForm($smarty, $arrFormFilter);
    $htmlFilter = $oFilterForm->fetchForm("/var/www/html/web/themes/iperfex3/_common/profile_uf.tpl",_tr('My Profile'), $dataProfile);
    $jsonObject = new PaloSantoJSON();
...
function handleJSON_deleteImageProfile($smarty, $module_name){
    ...
    $url="index.php?menu=_iperfexutils&action=getImage&ID=$idUser&rawmode=yes";
...
function handleJSON_getVideoPoster($smarty, $module_name)
{
    ...
    if ($domain == '') {
        ...
        $pDB = new iperfexDB($arrConf['iperfex_dsn']["iperfex"]);
        $paramSIP = getChatClientConfig($pDB, $error);
        $domain = $paramSIP['iperfex_chat_server'];
    }
...
function createProfileForm($langIperfex)
{   
    $arrFields = array(
