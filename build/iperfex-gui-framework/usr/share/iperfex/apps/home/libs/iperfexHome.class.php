...
/*
  @author: iperfexImap,v 1 2013/05/09 01:07:03 Washington Reyes wreyes@iperfexsanto.com Exp $
  @author: iperfexImap,v 2 2013/11/21 01:07:03 Rocio Mera rmera@iperfexsanto.com Exp $ */
class iperfexImap {
    private $user;
    private $port;
    private $host;
    ...
    public function iperfexImap($mailbox='INBOX',$host='',$port='', $default_folders=''){
        global $CYRUS;
    ...
    function readEmailMsg($uid){
        ...
        $pMessage=new iperfexImapMessage($this->connection,$uid,$this->mailbox);
...
*/
class iperfexImapMessage{
    private $app;
    private $iperfexImap; // connection open by imap_open
    private $uid = null;
    ...
    function iperfexImapMessage($imap,$uid,$mailbox){
        $this->iperfexImap=$imap;
        $this->uid=$uid;
    ...
    function imapReadMesg(){
        //get msg structure
        $this->structure = @imap_fetchstructure($this->iperfexImap, $this->uid, FT_UID);
    ...
    private function decodeData($p,$partno){
        // DECODE DATA
        $data = ($partno)?
            @imap_fetchbody($this->iperfexImap, $this->uid,$partno,FT_UID):  // multipart
            @imap_body($this->iperfexImap, $this->uid,FT_UID);  // simple
    ...
    function createAttachmentsToForward(){
        ...
        //4 agregamos a la variable de session[iperfex_emailAttachs] la informacion
    ...
    private function getpartAttachment($p,$partno,&$arrAttachments,$finfo){
        ...
        if ($p->ifdisposition){
            if ($p->disposition == "ATTACHMENT" || $p->disposition == "INLINE") {
                ...
                $message = @imap_fetchbody($this->iperfexImap, $this->uid, $partno, FT_UID);
                ...
                $_SESSION["iperfex_emailAttachs"][$idAttach]['name']=$filename;
                $_SESSION["iperfex_emailAttachs"][$idAttach]['mime']=$mimetype;
                $_SESSION["iperfex_emailAttachs"][$idAttach]['type']=$type;
            }
        }
        if (isset($p->parts)) {
            foreach ($p->parts as $partno0=>$p2)
    ...
    function downloadAttachment($partNum, $encoding) {
        $this->structure = @imap_bodystruct($this->iperfexImap, @imap_msgno($this->iperfexImap, $this->uid), $partNum);
    
        $filename = $this->structure->dparameters[0]->value;
        $message = @imap_fetchbody($this->iperfexImap, $this->uid, $partNum, FT_UID);
