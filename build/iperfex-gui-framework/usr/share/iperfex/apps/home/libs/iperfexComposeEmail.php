...
/*  @author: iperfexEmailAddress 2014/01/15 01:07:03 Rocio Mera rmera@iperfexsanto.com Exp $ */
require_once "apps/home/libs/emailaddress.php";
require_once "apps/home/libs/iperfexHome.class.php";
require_once "libs/phpmailer/class.phpmailer.php";

class iperfexComposeEmail{
    ...
    protected $pass='';
    // intancia de la calse iperfexImap
    protected $iperfexImap;
    ...
    function __construct($user, $pass, $name, &$pImap){
        ...
        if (filter_var($user, FILTER_VALIDATE_EMAIL)){
            //revisamos que el usuario exista en iperfex y obtenemos el nombre
            $this->username=$user;
        }else{
            $this->username=null;
        }
        ...
        //imap connection
        if($pImap instanceof iperfexEmailAddress){
            $this->iperfexImap=$pImap;
        }else{
            $this->iperfexImap=new iperfexImap();
        }
    }
    ...
    function sendEmail($headers,$subject,$body,$draft=false){
        ...
        try{
            ...
            //TO: creamos una nueva instacia de iperfexListEmailAddress con los emails
            //que se encuentran en el campo 'TO'
            $to=new iperfexListEmailAddress(explode(",",$headers['to']));
            ...
            //CC:
            if(isset($headers['cc'])){
                $cc=new iperfexListEmailAddress(explode(",",$headers['cc']));
                //ninguna de las direcciones dadas es válida, no podemos continuar
            ...
            if(isset($headers['bcc'])){
                $bcc=new iperfexListEmailAddress(explode(",",$headers['bcc']));
                //ninguna de las direcciones dadas es válida, no podemos continuar
                foreach($cc->getListAddress() as $address){
            ...
            if(isset($headers['reply_to'])){
                $replayTo=new iperfexListEmailAddress(explode(",",$headers['reply_to']));
                //ninguna de las direcciones dadas es válida, no podemos continuar
            ...
            $this->phpMailer->iperfexMsgHTML($body,'/var/www/html',$this->attachments);
            ...
            if($this->iperfexImap->login($this->username, $this->pass)){;
                $sentFolder='Sent';
                $arrFolders=$this->iperfexImap->getMailboxList($sentFolder);
                if(isset($arrFolders[$sentFolder])){
                    if(!$this->iperfexImap->createMailbox($sentFolder)){
                        $this->errMsg=$this->iperfexImap->getErrorMsg();
                    }
                }
                
                $this->iperfexImap->setMailbox('Sent');
                if(!$this->iperfexImap->appendMessage($sentFolder,$this->phpMailer->getMiMeStringToAppendImap(),"\\Seen")){
                    $this->errMsg=$this->iperfexImap->getErrorMsg();
                }
            }else{
                $this->errMsg=$this->iperfexImap->getErrorMsg();
            ...
            cleanAlertsImap();
            $this->iperfexImap->close_mail_connection();
            return true;
...
class PHPMailerMime extends PHPMailer{
     ...
     */
    public function iperfexMsgHTML($message, $basedir = '', $attachments, $advanced=FALSE)
    {
