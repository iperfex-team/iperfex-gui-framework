...
/*  @author: iperfexListEmailAddress 2014/01/15 01:07:03 Rocio Mera rmera@iperfexsanto.com Exp $ */
class iperfexListEmailAddress {
    //array of iperfexEmailAddress objects
    protected $listAddress = array();
    
    /**
     * received as parameter a array of email address
     * It can be an array of iperfexEmailAddress
    ...
    public function addAddress($email){
        if(is_string($email)){
            $address=new iperfexEmailAddress($email);
            if($address->getEmail()!=''){
                $this->add($address);
            }
        }elseif($email instanceof iperfexEmailAddress){
            if($email->getEmail()!=''){
                $this->add($email);
            }
        }
    ...
    //recibe una instacia de iperfexEmailAddress
