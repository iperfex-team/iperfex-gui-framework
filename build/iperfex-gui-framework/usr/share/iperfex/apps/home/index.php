...
require_once "libs/iperfexComposeEmail.php";

define('PATH_UPLOAD_ATTACHS','/var/www/iperfexdir/uploadAttachs');
/*
  @author: index.php,v 1 2013/05/09 01:07:03 Washington Reyes wreyes@iperfexsanto.com Exp $
  @author: index.php,v 2 2013/09/10 01:07:03 Rocio Mera rmera@iperfexsanto.com Exp $ */
...
function reportMail($smarty, $module_name, $local_templates_dir, & $arrConf, &$pImap)
{
    ...
    $result=$pImap->login($_SESSION['iperfex_user'], $_SESSION['iperfex_pass2']);
    if($result===false){
    ...
    if($total!=0){
        ...
        if($emails!==false){
            foreach($emails as $email){
                $tmp=array();
                $class='iperfex_unseen_email';
                if($email['SEEN']==1){
                    $class='iperfex_seen_email';
                }
                $tmp[]="<div class='iperfex_row $class' id={$email['UID']}>";
                $tmp[]="<div class='ic'>";
                ...
                $class='iperfex_unflagged_email';
                if($email['FLAGGED']==1){
                    $class='iperfex_flagged_email';
                }
                $tmp[]="<div class='star'><span class='$class glyphicon glyphicon-star'></span></div>";
                $tmp[]="</div>";
                //$tmp[]="<div class='email_msg_attr'>";
                $tmp[]="<div class='from  iperfex_row_email_msg'> <span>".htmlentities($email['from'],ENT_COMPAT,'UTF-8')."</span></div>";
                $tmp[]="<div class='subject iperfex_row_email_msg'> <span>".htmlentities($email['subject'],ENT_COMPAT,'UTF-8')."</span></div>";
                $tmp[]="<div class='date iperfex_row_email_msg'><span>".$email['date']."</span></div>";
                //$tmp[]="</div>";
                $tmp[]="</div>";
                $arrData[]=$tmp;
            }
...
function download_attach($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    if($result===false){
        ...
        $pMessage=new iperfexImapMessage($pImap->getConnection(),$uid,$mailbox);
...
function inline_attach($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    $pMessage=new iperfexImapMessage($pImap->getConnection(),$uid,$pImap->getMailbox());
...
function get_templateEmail($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    //eliminamos el contenido de la variable de session iperfex_emailAttachs
    //esta variable contiene el nombre de los archivos atachados
    unset($_SESSION['iperfex_emailAttachs']);
...
function compose_email($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    $pCompose = new iperfexComposeEmail($_SESSION['iperfex_user'], $_SESSION['iperfex_pass2'], $name, $pImap);
    ...
    $attachments=null;
    if(isset($_SESSION['iperfex_emailAttachs'])){
        $pCompose->setAttachments($_SESSION['iperfex_emailAttachs']);
    }
...
function attachFile($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    try {
        ...
        $_SESSION["iperfex_emailAttachs"][$idAttach]['filename']=$tmpFileName;
        $_SESSION["iperfex_emailAttachs"][$idAttach]['name']=$_FILES['attachFileButton']['name'][0];
        $_SESSION["iperfex_emailAttachs"][$idAttach]['mime']=$mimetype;
        $_SESSION["iperfex_emailAttachs"][$idAttach]['type']="file";
...
function dettachFile($smarty, $module_name, $local_templates_dir,  $arrConf, &$pImap){
    ...
    try {
        if(is_string($idAttach) && $idAttach!=''){
            ...
            $filename=$_SESSION['iperfex_emailAttachs'][$idAttach]['filename'];
