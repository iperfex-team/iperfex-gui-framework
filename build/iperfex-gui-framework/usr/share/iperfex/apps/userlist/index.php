...
+----------------------------------------------------------------------+
  ...
  +----------------------------------------------------------------------+
  | Copyright (c) 2006 Iperfex Solutions S. A.                         |
  +----------------------------------------------------------------------+
...
//	include_once "libs/iperfexFax.class.php";
...
//        }
    ...
    // if($pORGZ->getNumOrganization(array()) > 0){
        ...
        if($credentials['userlevel']=="superadmin"){
            $organizations = $pORGZ->getOrganization(array());
            if($organizations == "")
                $organizations = array();
        ...
        $oFilterForm = new iperfexForm($smarty, $arrFormElements);
        $oGrid->addFilterControl(_tr("Filter applied ")._tr("Username")." = ".$username, $_POST, array("username" => "")); //username
        $htmlFilter = $oFilterForm->fetchForm("$local_templates_dir/filter.tpl", "", $_POST);
        $oGrid->showFilter(trim($htmlFilter));
    // }else{
        // $smarty->assign("mb_title", _tr("MESSAGE"));
        // $smarty->assign("mb_message",_tr("In order to use this module must exist at least 1 organization in the Iperfex Server"));
    // }
...
{
    require_once("libs/iperfexEmail.class.php");
 
    $pACL = new iperfexACL(new iperfexDB($arrConf['iperfex_dsn']['acl']));
    $pEmail = new iperfexEmail($pDB);
    $jsonObject = new IperfexJSON();
    $username=getParameter('username');
...
function viewFormUser($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $credentials){
    ...
    if($action=="view" || getParameter("edit") || getParameter("save_edit")){
        ...
        if($arrUsers===false){
			...
			$arrFill["country_code"] = $pORGZ->getOrganizationProp($arrFill["organization"], "country_code");
			$arrFill["area_code"] = $pORGZ->getOrganizationProp($arrFill["organization"], "area_code");
            
            // // ahora obtenemos las configuraciones de fax dle usuario
            // $pFax=new iperfexFax($pACL->_DB);
            // $listFaxs=$pFax->getFaxList(array("exten"=>$extf,"organization_domain"=>$arrDomains[$arrFill["organization"]]));
			// print_r($listFaxs);die;
            // if($listFaxs!=false){
                // $faxUser=$listFaxs[0];
...
function saveNewUser($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $arrCredentiasls){
    ...
    if($exito){
        ...
        if(isset($_FILES['picture']['name']) && $_FILES['picture']['name'] != ""){
            $newUserId=$pACL->getUserByUsername($username."@iperfex.com");
          
            uploadImage($newUserId["id"],$pDB,$errorImg);
        }
        $newUserId=$pACL->getUserByUsername($username."@iperfex.com");
        $smarty->assign("mb_title", _tr("MESSAGE"));
        $smarty->assign("mb_message",_tr("User has been created successfully")."</br>");
...
function deleteUser($smarty, $module_name, $local_templates_dir, $pDB, $arrConf, $arrCredentiasls){
    ...
    if($arrCredentiasls['userlevel']=="superadmin"){
        if($idUser==1){
            $smarty->assign("mb_title", _tr("ERROR"));
            $smarty->assign("mb_message",_tr("The admin user cannot be deleted because is the default Iperfex administrator. You can delete any other user."));
            return reportUser($smarty, $module_name, $local_templates_dir, $pDB, $arrConf, $arrCredentiasls);
        }else{
...
function getImage($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $arrCredentiasls){
    $pACL       = new iperfexACL($pDB);
    $imgDefault = $_SERVER['DOCUMENT_ROOT']."/".$arrConf['adminFolder']."/web/apps/$module_name/images/Icon-user.png";
