<?php
  ...
  +----------------------------------------------------------------------+
  $Id: index.php,v 1.1 2012-02-07 11:02:12 Rocio Mera rmera@iperfexsanto.com Exp $ */
...
include_once "libs/iperfexJSON.class.php";

function _moduleContent(&$smarty, $module_name)
{
    global $arrConf;
    
#    include_once "apps/did/libs/iperfexDID.class.php";
    include_once "libs/iperfexOrganization.class.php";
    include_once "libs/iperfexEmail.class.php";
	//include_once "libs/iperfexAsteriskConfig.class.php";
	include_once "libs/extensions.class.php";
	include_once "libs/iPerfexCampaignOutbound.class.php";
    // include_once "libs/iPerfexCampaignInbound.class.php";
    // include_once "libs/iPerfexManageriSDXConfig.class.php";
	// include_once "libs/iPerfexCallTimeTemplates.class.php";
    // include_once "libs/iPerfexRecyclingTemplates.class.php";
    // include_once "libs/iPerfexQuestionnaire.class.php";
...
function reportOrganization($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $credentials)
{
    ...
    $pOrganization = new iperfexOrganization($pDB);
	...
	$totalchannels = $pOrganization->getOrganizationsChannels();
...
//    $arrColumns[]=_tr("Email Qouta")." (MB)";
	 ...
	 if($credentials["userlevel"]=="superadmin")
		 $arrColumns[]=_tr("Channels")." (".$totalchannels."/".$arrConf['_LICENCE_CHANNELS_IPERFEX'].")";
	else
		 $arrColumns[]=_tr("Channels");
...
//            $arrTmp[] = ($eQuota===false)?_tr("NONE"):$eQuota;
	...
	if($credentials['userlevel']=="superadmin"){
        ...
        $oFilterForm = new iperfexForm($smarty, $arrFormFilter);
...
function change_state($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $credentials){
    ...
    $pOrg = new iperfexOrganization($pDB);
...
function showMessageReload($module_name, &$pDB, $credentials){
    $pDBMySQL=new iperfexDB(generarDSNSistema("asteriskuser", "iperfexpbx"));
//    $pAstConf=new iperfexASteriskConfig($pDB);
    $params=array();
    $msgs="";
...
function viewFormOrganization($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $credentials)
{
    ...
    if($action=="view" || getParameter("edit") || getParameter("save_edit")){ 
        ...
        if(is_array($dataOrgz) & count($dataOrgz)>0){
            ...
            if(!getParameter("save_edit")){
				...
				$arrFill['channels'] = $pOrganization->getOrganizationProp($id ,"channels");
            }
        ...
        if(getParameter("new_organization")){
            $arrFill['quota'] = 30;
			$arrFill['channels'] = $arrConf['_LICENCE_CHANNELS_IPERFEX'];
            $check_e="checked";
            $check_u="checked";
            $check_q="checked";
        }
   ...
   // $smarty->assign("icon", "web/apps/organizaciones/images/organization.png");
    ...
    $arrFormOrgz = createFieldForm($pDB);
    $oForm = new iperfexForm($smarty,$arrFormOrgz);
    if($action=="view"){
        $oForm->setViewMode();
...
//        $num_queues = isset($_POST["max_num_queues_chk"])?"0":getParameter("max_num_queues");
	    ...
	    $channels = trim(getParameter("channels"));
...
//            $num_queues=$num_queues+0;
		...
		// channels validation
        $totalChannelsAssigned=$pOrganization->countAssignedChannels();
        $totalChannelsAvailable=$arrConf['_LICENCE_CHANNELS_IPERFEX']-$totalChannelsAssigned;
        if ($channels > $totalChannelsAvailable)
            $error=sprintf(_('You have %s channels left available in your license of %s. Please remove channels from other organizations to add more channels to this organization.'), $totalChannelsAvailable, $arrConf['_LICENCE_CHANNELS_IPERFEX']);
        if ($channels < 0)
            $error=_tr('Channels must be higher than 1');
        if($error!=""){
            $smarty->assign("mb_title", _tr("Error"));
        ...
        $exito=$pOrganization->createOrganization($name,$domain,$country,$state,$address,$country_code,$area_code,$num_user,$admin_password, $email_contact,$channels);
        if($exito!==false){
            $smarty->assign("mb_title", _tr("Message"));
            $smarty->assign("mb_message", _tr("The organization was created successfully")."<br />"._tr("To admin the new organization login to iperfex as admin@").$domain.$pOrganization->errMsg);
            return reportOrganization($smarty, $module_name, $local_templates_dir, $pDB, $arrConf, $credentials);
        }else{
            $smarty->assign("mb_title", _tr("Error"));
            $smarty->assign("mb_message",_tr($pOrganization->errMsg));
...
{
    $pOrganization = new iperfexOrganization($pDB);
    $iCampaignOutbound = new iPerfexCampaignOutbound($pDB);
    $arrFormOrgz = createFieldForm($pDB);
    $oForm = new iperfexForm($smarty,$arrFormOrgz);
    $error = "";
...
//        $email_contact = trim(getParameter("email_contact"));
        ...
        // channels validation
        $totalChannelsAssigned=$pOrganization->countAssignedChannels($idOrg);
        $totalChannelsAvailable=$arrConf['_LICENCE_CHANNELS_IPERFEX']-$totalChannelsAssigned;
        if ($channels > $totalChannelsAvailable)
            $error=sprintf(_('You have %s channels left available in your license of %s. Please remove channels from other organizations to add more channels to this organization.'), $totalChannelsAvailable, $arrConf['_LICENCE_CHANNELS_IPERFEX']);
        if ($channels < 0)
        ...
        // validate total channels are higher than the amount of channels available in active campaigns
        $activeChannels = $iCampaignOutbound->countActiveChannels($idOrg);
        if ($activeChannels > $channels)
            $error=sprintf(_tr('You must disable some campaigns before changing channels amount. You have %s channels in active campaigns.'), $activeChannels);
        ...
        if($country=="0" || !isset($country)){
            ...
            $exito=$pOrganization->setOrganization($idOrg,$name,$country,$city,$address,$country_code,$area_code,$quota,$num_user,$num_exten,$num_queues,$credentials['userlevel'],$channels);
            if($exito)
            {
                $smarty->assign("mb_title", _tr("Message"));
...
function createFieldForm(&$pDB)
{
    ...
    $maxChannels = $arrConf['_LICENCE_CHANNELS_IPERFEX'] - $totalchannels;
    $arrFields = array(
			...
			"channels"   => array(
                "LABEL"                  => _tr('Channels'),
                "DESCRIPTION"            => _tr("Channels"),
                "REQUIRED"               => "yes",
                "INPUT_TYPE"             => "TEXT",
                "INPUT_EXTRA_PARAM"      => array("class"=>"form-control channels","max"=>$maxChannels),
...
function reloadAsterisk($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $credentials){
    ...
    if($continue){
       /* $pAstConf=new iperfexASteriskConfig($pACL->_DB);
...
function reportDIDorganization($smarty, $module_name, $local_templates_dir, $pDB, $arrConf, $credentials){
    $pORGZ = new iperfexOrganization($pDB);
    $pDID=new iperfexDidPBX($pDB);
