...
function _moduleContent($smarty, $module_name)
{
    ...
    $iperfexApplets = new iperfexApplets();
    $appletlist = $iperfexApplets->leerAppletsActivados($_SESSION["iperfex_user"]);
    $t = array();
    foreach ($appletlist as $applet) {
    	if (is_dir("{$arrConf['iperfex']}/apps/$module_name/applets/{$applet['applet']}"))
            $t[] = $applet;
    }
    $appletlist = $t;
    ...
    if (isset($_REQUEST['applet'])) {
        if (file_exists("{$arrConf['iperfex']}/apps/$module_name/applets/{$_REQUEST['applet']}/lang/en.lang"))
            load_language_module("$module_name/applets/{$_REQUEST['applet']}");
        require_once "{$arrConf['iperfex']}/apps/$module_name/applets/{$_REQUEST['applet']}/index.php";
    }
...
function handleJSON_updateOrder($smarty, $module_name, $appletlist)
{
    ...
    if (!isset($_REQUEST['appletorder']) || !is_array($_REQUEST['appletorder'])) {
        ...
        $iperfexApplets = new iperfexApplets();
        if (!$iperfexApplets->actualizarOrdenApplets($_SESSION["iperfex_user"], $_REQUEST['appletorder'])) {
            $respuesta['status'] = 'error';
            $respuesta['message'] = $iperfexApplets->errMsg;
        }
    }
