...
class Applet_ProcessesStatus
{
    ...
    private function _controlServicio($sAccion)
    {
    	...
    	if (!isset($_REQUEST['process'])) {
            ...
            $pDBACL = new iperfexDB($arrConf['iperfex_dsn']['acl']);
            if (!empty($pDBACL->errMsg)) {
                ...
                if (!empty($pACL->errMsg)) {
                ...
                } elseif (!$pACL->isUserSuperAdmin($_SESSION['iperfex_user'])) {
                    $respuesta['status'] = 'error';
                    ...
                    if (($sAccion == 'off') || ($sAccion == 'on')) {
                        exec('/usr/bin/iperfex-helper rchkconfig --level 3 '.
    ...
    {   // file pid service asterisk    is /var/run/asterisk/asterisk.pid
        ...
        // file pid service call_center is /opt/iperfex/dialer/dialerd.pid
/*
...
*/
        ...
        $arrSERVICES["MySQL"]["status_service"]    = $this->_existPID_ByCMD("mysqld","mariadb");
        $arrSERVICES["MySQL"]["activate"]      = $this->_isActivate("mariadb");
        ...
        $arrSERVICES["Dialer"]["status_service"]   = $this->_existPID_ByFile("/opt/iperfex/dialer/dialerd.pid","iperfexdialer");
        $arrSERVICES["Dialer"]["activate"]     = $this->_isActivate("iperfexdialer");
        $arrSERVICES["Dialer"]["name_service"]     = "Iperfex Call Center Service";
    ...
    {
        if (!$this->_existService($nameService)) return "Not_exists";
        foreach (explode(' ', trim(`/usr/sbin/pidof $serviceName`)) as $pid) {
