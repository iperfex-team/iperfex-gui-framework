<?php
  ...
  +----------------------------------------------------------------------+
  $Id: WSDLcreator.class.php,v 1.0 2011-03-18 15:30:00 Bruno Macias V.  bmacias@iperfex.org Exp $*/
...
class WSDLcreator
{
    ...
    /**
     * Prefix namespace for iperfex
     *
     * @var object
     */
    private $prefix_ns_iperfex;
     ...
     */
    public function WSDLcreator($nameWSDL, $targetNamespace="http://www.iperfex.com/webservices", $soapAddress)
    {
        /*
        ...
        */
        ...
        $this->prefix_ns_iperfex    = "iperfex";
    ...
    private function createWSDL_TPL()
    {
        ...
        // iperfex  for namespace (iperfex), ex. http://www.iperfex.com/webservices/
        try{
            ...
            $doc->createAttributeNS($this->targetNamespace,                  "{$this->prefix_ns_iperfex}:iperfex");
            ...
            $objBinding->addAttribute("type","{$this->prefix_ns_iperfex}:{$this->nameWSDL}_SOAPPort");
            ...
            $objPort->addAttribute("binding","{$this->prefix_ns_iperfex}:{$this->nameWSDL}_SOAP");
        ...
        try{
            foreach($arrParams as $param => $arrParamType){
                ...
                if($this->setSchemaTypes($name1, $arrParamType)){
                    ...
                    $objPar->addAttribute("element","{$this->prefix_ns_iperfex}:$name1");
                }
                else return false;
            }
        }
    ...
    private function setPortType_Operation($function)
    {
        ...
        try{
            ...
            $objIN->addAttribute( "message","{$this->prefix_ns_iperfex}:{$function}_Request");
            $objOUT->addAttribute("message","{$this->prefix_ns_iperfex}:{$function}_Response");
        }
        catch(Exception $ex){
