...
function load_version_iperfex()
{
    global $iperfex;
    require_once "$iperfex/configs/default.conf.php";
    include_once "$iperfex/libs/iperfexDB.class.php";
    global $arrConf;
    //conectarse a la base de settings para obtener la version y release del sistema iperfex
    $pDB = new iperfexDB($arrConf['iperfex_dsn']['iperfex']);
    if(empty($pDB->errMsg)) {
        $theme=get_key_settings($pDB,'iperfex_version_release');
    }
//si no se encuentra setear solo ?
    if (empty($theme)){
        set_key_settings($pDB,'iperfex_version_release','?');
        return "?";
    }
    else return $theme;
}
...
function load_theme()
{
    ...
    global $arrConf;
    $pDB = new iperfexDB($arrConf['iperfex_dsn']['iperfex']);
    $user = isset($_SESSION['iperfex_user'])?$_SESSION['iperfex_user']:"";
    $theme=null;
    if(empty($pDB->errMsg)) {
        if($user==""){
...
function update_theme()
{
    ...
    //Update menus iperfex permission.
...
function load_language()
{
    ...
    include_once "$iperfex/lang/en.lang";
    $lang_file = "$iperfex/lang/$lang.lang";
...
function load_language_module($module_id)
{
    ...
    $lang = get_language();
    include_once "$iperfex/apps/$module_id/lang/en.lang";
    $lang_file_module = "$iperfex/apps/$module_id/lang/$lang.lang";
...
function get_language()
{
    global $iperfex;
    require_once "$iperfex/configs/default.conf.php";
    include "$iperfex/configs/languages.conf.php";
    include_once "$iperfex/libs/iperfexOrganization.class.php";
...
/**
 * Función para obtener la clave del Cyrus Admin de Iperfex.
 * La clave es obtenida de /etc/iperfex.conf
 *
...
{
    global $iperfex;
    require_once "$iperfex/libs/iperfexConfig.class.php";

	$pConfig = new iperfexConfig("/etc", "iperfex.conf", "=", "[[:space:]]*=[[:space:]]*");
	$listaParam = $pConfig->leer_configuracion(FALSE);
	if (isset($listaParam['cyrususerpwd'])) 
		return $listaParam['cyrususerpwd']['valor'];
	else return 'iperfexsanto'; // Compatibility for updates where /etc/iperfex.conf is not available
}

/**
 * Función para obtener la clave MySQL de usuarios bien conocidos de Iperfex.
 * Los usuarios conocidos hasta ahora son 'root' (sacada de /etc/iperfex.conf)
...
function obtenerClaveConocidaMySQL($sNombreUsuario)
{
    ...
    case 'root':
        $pConfig = new iperfexConfig("/etc", "iperfex.conf", "=", "[[:space:]]*=[[:space:]]*");
        $listaParam = $pConfig->leer_configuracion(FALSE);
        if (isset($listaParam['MYSQL_ROOT_PASSWORD']))
            return $listaParam['MYSQL_ROOT_PASSWORD']['valor'];
        break;
    case 'asteriskuser':
        $pConfig = new iperfexConfig("/etc", "iperfex.conf", "=", "[[:space:]]*=[[:space:]]*");
        $listaParam = $pConfig->leer_configuracion(FALSE);
...
/**
 * Función para obtener la clave AMI del usuario admin, obtenida del archivo /etc/iperfex.conf
 *
 * @param   string  $ruta_base          Ruta base para inclusión de librerías
 *
 ...
 * frecuentemente utilizadas en Iperfex. Para cada base de datos reconocida, se
 * busca la clave en /etc/iperfex.conf o en /etc/amportal.conf según corresponda.
 *
 * @param   string  $sNombreUsuario     Nombre de usuario para interrogar
 ...
 */
function generarDSNSistema($sNombreUsuario, $sNombreDB, $sHost = NULL)
{
    global $iperfex;
    ...
    case 'root':
        ...
        //echo 'mysql://root:'.$sClave.'@'.$sHost.'/'.$sNombreDB;
        return 'mysql://root:'.$sClave.'@'.$sHost.'/'.$sNombreDB;
    case 'asteriskuser':
        $sClave = obtenerClaveConocidaMySQL($sNombreUsuario);
        if (is_null($sClave)) 
            return NULL;
        else{
            //echo "mysql://asteriskuser:".$sClave.'@'.$sHost.'/'.$sNombreDB;
            return "mysql://asteriskuser:".$sClave.'@'.$sHost.'/'.$sNombreDB;
        }
    }
    return NULL;
}

//function isPostfixToIperfex2(){
...
//        $band = TRUE; // if the conf postfix is for Iperfex 2.0
//    }
//    else{
//        $band = FALSE;// if the conf postfix is for Iperfex 1.6
//    }
//    return $band;
//}

// Esta función revisa las bases de datos del framework (iperfex.db, register.db, samples.db) en caso de que no existan y se encuentre su equivalente pero con extensión .rpmsave entonces se las renombra.
 ...
 * 
 * @param   object  $pdbACL     Objeto iperfexDB que contiene la coneccion a la base iperfexpbx.
...
function putMenuAsHistory($pdbACL, $uid, $id_resource)
{
    ...
    $pDB = new iperfexDB($arrConf['iperfex_dsn']['iperfex']);
    if (empty($pDB->errMsg)) {
        $uiperfex = get_key_settings($pDB, 'uiperfex');
        if ((int)$uiperfex != 0) return TRUE;
    }
 ...
 * @author ecueva@iperfexsanto.com
 */
...
function AsteriskManagerConnect(&$error) {
	...
	$pConfig = new iperfexConfig("/var/www/iperfexdir/asteriskconf", "/iperfex_pbx.conf", "=", "[[:space:]]*=[[:space:]]*");
...
function getUserCredentials($username){
    global $arrConf,$iperfex;
    require_once("$iperfex/libs/iperfexACL.class.php");
    $pdbACL = new iperfexDB($arrConf['iperfex_dsn']['iperfex']);
    $pACL = new iperfexACL($pdbACL);
...
{   
    global $iperfex;
    global $arrConf;
    if (file_exists('/usr/share/php/Smarty/SmartyBC.class.php'))
        require_once('Smarty/SmartyBC.class.php');
    else require_once("$iperfex/libs/smarty/libs/SmartyBC.class.php");     
    $smarty = new SmartyBC();     
    
    $mainTheme=basename($mainTheme);
    
    $smarty->template_dir = "{$arrConf['basePath']}/web/themes/$mainTheme";   
    $smarty->config_dir =   "$iperfex/configs/";  
    
    if(!is_dir("{$arrConf['documentRoot']}/tmp/smarty/templates_c/$mainTheme")){
        mkdir("{$arrConf['documentRoot']}/tmp/smarty/templates_c/$mainTheme", 0777, true);
    }
    if(!is_dir("{$arrConf['documentRoot']}/tmp/smarty/cache/$mainTheme")){
        mkdir("{$arrConf['documentRoot']}/tmp/smarty/cache/$mainTheme", 0777, true);
    }
