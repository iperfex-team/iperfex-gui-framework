...
+----------------------------------------------------------------------+
  ...
  +----------------------------------------------------------------------+
$Id: iperfexEmail.class.php,v 1.1.1.1 2007/07/06 21:31:55 gcarrillo Exp $ */
...
//include_once("$iperfex/configs/email.conf.php");
//include_once("$iperfex/libs/cyradm.php");
...
class iperfexEmail {
    ...
    */
    ...
    function createAccount($domain, $username, $password)
    {
        $this->errMsg = '';
        $output = $retval = NULL;
        $sComando = '/usr/bin/iperfex-helper email_account --createaccount'.
    ...
    function deleteAccount($username)
    {
        $this->errMsg = '';
        $output = $retval = NULL;
        $sComando = '/usr/bin/iperfex-helper email_account --deleteaccount --username '.
...
//    }
    ...
    function setAccountPassword($username, $password)
    {
        $this->errMsg = '';
        $output = $retval = NULL;
        $sComando = '/usr/bin/iperfex-helper email_account --setaccountpassword'.
            ' --username '.escapeshellarg($username).
            ' --password '.escapeshellarg($password).
            ' 2>&1';
        exec($sComando, $output, $retval);
...
//        else
        ...
        if(!is_null($username)){
            if(!preg_match($regularExpresion,$username)){
                    ...
                    exec('/usr/bin/iperfex-helper email_account --reconstruct_mailbox  --mailbox '.escapeshellarg($username).' 2>&1', $output, $retval);
            }
        }else{
    ...
    function reloadPostfix(){
        $sComando = '/usr/bin/iperfex-helper email_account --reloadPostfix 2>&1';
        $output = $ret = NULL;
    ...
    function writePostfixMain(){
        $sComando = '/usr/bin/iperfex-helper email_account --writePostfixMain 2>&1';
        $output = $ret = NULL;
        exec($sComando, $output, $ret);
        if ($ret != 0){
            $this->errMsg = implode('', $output);
