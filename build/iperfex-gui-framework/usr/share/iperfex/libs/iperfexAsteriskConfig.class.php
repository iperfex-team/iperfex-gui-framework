<?php
  ...
  +----------------------------------------------------------------------+
  $Id: iperfexAsteriskConfig,v 1.1 05/11/2012 rocio mera rmera@iperfexsanto.com Exp $ */

$iperfex="/usr/share/iperfex";
include_once "$iperfex/libs/iperfexConfig.class.php";
include_once "$iperfex/libs/misc.lib.php";

global $arrConf;

class iperfexASteriskConfig{
    public $errMsg;
    public $_DB; //conexion a la base iperfexpbx mysql

    //recibe una conexion a la base de iperfexpbx de mysql
    function iperfexASteriskConfig(&$pDB)
    {
    ...
    function writeExtesionConfFile(){
        $sComando = '/usr/bin/iperfex-helper asteriskconfig writeExtensionConf 2>&1';
        $output = $ret = NULL;
    ...
    function createOrganizationAsterisk($domain,$country){
        require_once "apps/features_code/libs/iperfexFeaturesCode.class.php";
        ...
        $pFC=new iperfexFeatureCodePBX($this->_DB,$domain);
        ...
        if($this->setGeneralSettingFirstTime($domain,$orgCode,$country)){
            if($pFC->insertPaloFeatureDB()){
                if($this->setReloadDialplan($domain)){
                    //realizamos la
                    $sComando = '/usr/bin/iperfex-helper asteriskconfig createOrganizationAst '.
    ...
    {   
        ...
        include_once "libs/iperfexPBX.class.php";
        
        global $arrConf;
        $source_file="/var/www/iperfexdir/asteriskconf/globals.conf";
        
        $pGlobals=new iperfexGlobalsPBX($this->_DB,$domain);
        ...
        $psip=new iperfexSip($this->_DB);
        $piax=new iperfexIax($this->_DB);
        $pvoicemail=new iperfexVoicemail($this->_DB);
    ...
    */
    ...
    function generateDialplan($domain,$reload=false){
        ...
        $sComando = "/usr/bin/iperfex-helper asteriskconfig generateDialPlan ".
...
class iperfexContexto{
    ...
    function iperfexContexto($code,$name){
        global $arrConf;
...
class iperfexExtensions{
	...
	function iperfexExtensions($extension,$application,$priority="",$label=""){
