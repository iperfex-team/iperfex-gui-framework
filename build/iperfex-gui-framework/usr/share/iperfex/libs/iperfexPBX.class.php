<?php
  ...
  +----------------------------------------------------------------------++
  $Id: iperfexPBX.class.php,v 1.1 2012/07/30 rocio mera rmera@iperfexsanto.com Exp $ */

global $arrConf;
include_once "{$arrConf['iperfex']}/libs/iperfexACL.class.php";
include_once "{$arrConf['iperfex']}/libs/iperfexConfig.class.php";
include_once "{$arrConf['iperfex']}/libs/iperfexAsteriskConfig.class.php";
include_once "{$arrConf['iperfex']}/libs/extensions.class.php";
include_once "{$arrConf['iperfex']}/libs/iperfexIM.class.php";
include_once "{$arrConf['iperfex']}/libs/misc.lib.php";

if (file_exists("/var/lib/asterisk/agi-bin/phpagi-asmanager.php")) {
    require_once "/var/lib/asterisk/agi-bin/phpagi-asmanager.php";
}

class iperfexAsteriskDB {
    public $_DB;
    public $errMsg;
...
class iperfexSip extends iperfexAsteriskDB {
    ...
    // Este método es público sólo para poderlo invocar desde iperfexTrunk
	...
	//esta funcion solo debe ser llamada desde iperfexdevice
...
class iperfexIax extends iperfexAsteriskDB {
	public $name;
	public $type; //="friend";
	public $username;
	public $mailbox;
...
class iperfexVoicemail extends iperfexAsteriskDB{
    public $context;
...
class iperfexDevice{
    ...
    function iperfexDevice($domain,$type,$pDB)
    {
        if(!preg_match("/^(([[:alnum:]-]+)\.)+([[:alnum:]])+$/", $domain)){
            $this->errMsg="Invalid domain format";
        }else{
            ...
            if($type=="sip"){
                $this->tecnologia=new iperfexSip($pDB);
                $this->errMsg=$this->tecnologia->errMsg;
            }elseif($type=="iax2"){
                $this->tecnologia=new iperfexIax($pDB);
                $this->errMsg=$this->tecnologia->errMsg;
            }else{
            ...
            if($result==false){
                $this->errMsg .=_tr("Can't create a new instace of iperfexDevice").$this->tecnologia->errMsg;
            }else{
    ...
    function validatePaloDevice(){
        //validamos que la instancia de iperfexDevice que se esta usando haya sido creda correctamente
    ...
    */
    function createNewDevice($arrProp,$type)
    {
        ...
        if($type=="iax2")
            $this->tecnologia=new iperfexIax($this->tecnologia->_DB);
        elseif($type=="sip")
            $this->tecnologia=new iperfexSip($this->tecnologia->_DB);
        else{
        ...
        if($arrProp['create_vm']=="yes"){
            ...
            $pVM=new iperfexVoicemail($this->tecnologia->_DB);
        ...
        $arrProp["iperfexweb_device"]=NULL;
        if($type=='sip'){ 
            $arrProp["iperfexweb_device"]=NULL;
            if(isset($arrProp["create_iperfexweb_device"])){
                if((bool)$arrProp["create_iperfexweb_device"]){
                    $arrProp["enable_chat"]='yes';
                    $arrProp["iperfexweb_device"]="{$arrProp['name']}IM_{$this->code}";
                }
            }
        }
        ...
        $arrExten['iperfexweb_device']=$arrProp["iperfexweb_device"];
        ...
        if(isset($arrProp["create_iperfexweb_device"]) && $type=='sip'){ 
            $idExten=$this->tecnologia->_DB->getLastInsertId();
            //creamos el peer sip y registramos que existe una cuenta para chatear en la tabla iperfex-im 
            $IM=$arrProp;
            ...
            $pIM=new iperfexIM($this->tecnologia->_DB,$this->domain);
    ...
    {
        $query="INSERT INTO extension (organization_domain,tech,dial,exten,device,rt,record_in,record_out,context,voicemail,outboundcid,alias,mohclass,noanswer,enable_chat,iperfexweb_device,clid_name,clid_number) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $result=$this->tecnologia->executeQuery($query,array($this->domain,$arrExten['tech'],$arrExten['dial'],$arrExten['exten'],$arrExten['device'],$arrExten['rt'],$arrExten['record_in'],$arrExten['record_out'],$arrExten['context'],$arrExten['voicemail'],$arrExten['outboundcid'],$arrExten['alias'],$arrExten['mohclass'],$arrExten['noanswer'],$arrExten['enable_chat'],$arrExten['iperfexweb_device'],$arrExten['clid_name'],$arrExten['clid_number']));
        if($result==false)
            $this->errMsg=$this->tecnologia->errMsg;
        return $result; 
    }
    ...
    private function insertDeviceASTDB($arrProp)
    {
        ...
        $arrSetting["iperfexweb_device"]=(isset($arrProp["iperfexweb_device"]))?$arrProp["iperfexweb_device"]:NULL;
        if(!empty($arrSetting["iperfexweb_device"])){
            $arrSetting["device"] .="&".$arrSetting["iperfexweb_device"];
        }
        ...
        $family_device_im = "DEVICE/$code/{$arrSetting["iperfexweb_device"]}";
        $arrInsert=array();
        ...
        if($arrSetting["iperfexweb_device"]!=''){ 
            ...
            $arrKeyDevice["dial"]="SIP/{$arrSetting["iperfexweb_device"]}";
            foreach($arrKeyDevice as $key => $value){
    ...
    function editDevice($arrProp){
        ...
        if($result!=false){
            ...
            //si la cuenta sip tiene dispositivo iperfexweb_device entonces se lo agrega
            if($tech=='sip'){
                //si la cuenta sip tiene dispositivo iperfexweb_device entonces debe ser actualizado
                if(isset($arrProp["iperfexweb_device"])){
                    if(!empty($arrProp["iperfexweb_device"])){
                        $IM=$arrProp;
                        $IM['name'] = $arrProp["iperfexweb_device"];
                        $IM['secret'] = $secret;
                        $IM["outofcall_message_context"] = 'im-sip';
                        $IM["subscribecontext"] = 'im-sip';
                        $IM['id_exten']=$idExten;    //este campo es para indicar que la cuenta sip usada 
            ...
            $arrExten['mohclass']='';
            $arrExten['noanswer']='';
            $arrExten['iperfexweb_device']=isset($arrProp["iperfexweb_device"])?$arrProp["iperfexweb_device"]:NULL;
    ...
    function editExtensionDB($arrProp){
        $query="UPDATE extension SET rt=?, record_in=?, record_out=?, context=?, voicemail=?, outboundcid=?, alias=?, mohclass=?, noanswer=?, enable_chat=?, iperfexweb_device=?, clid_name=?, clid_number=? where exten=? and organization_domain=?";
        $result=$this->tecnologia->executeQuery($query,array($arrProp["rt"],$arrProp["record_in"],$arrProp["record_out"],$arrProp["context"],$arrProp["voicemail"],$arrProp["outboundcid"],$arrProp["alias"],$arrProp["mohclass"],$arrProp["noanswer"],$arrProp["enable_chat"],$arrProp["iperfexweb_device"],$arrProp["clid_name"],$arrProp["clid_number"],$arrProp['exten'],$this->domain));
        if($result==false)
            $this->errMsg=$this->tecnologia->errMsg;
        return $result; 
    }
    ...
    function changePasswordExtension($password,$exten){
        ...
        if($result==false){
            ...
            if(!$this->tecnologia->setSecret($result["device"],$password,$this->domain)){
                ...
                //esto se identifica por el campo iperfexweb_device. A esta cuenta tambien ahi que actualizarle
                //el password
                if($result["tech"]=="sip" && !empty($result["iperfexweb_device"])){
                    if(!$this->tecnologia->setSecret($result["iperfexweb_device"],$password,$this->domain)){
                        $this->errMsg=$this->tecnologia->errMsg;
                        return false;
                    }
                }
    ...
    function deleteExtension($extension){
        ...
        $query="Select id, organization_domain, exten, device, tech, voicemail, iperfexweb_device from extension where exten=? and organization_domain=?";
        ...
        if($result==false && $this->tecnologia->errMsg!="Don't exist extension $extension. "){
            ...
            if(is_array($result) && count($result)>0){
                ...
                if(isset($result["voicemail"]) && $result["voicemail"]!="novm"){
                    $pVoicemail= new iperfexVoicemail($this->tecnologia->_DB);
                ...
                if(!empty($result["iperfexweb_device"])){
                    $query="DELETE FROM im WHERE device=? and organization_domain=?";
                    $exito=$this->tecnologia->executeQuery($query,array($result["iperfexweb_device"],$this->domain));
                    if(!$exito){
                        $this->errMsg="Extension can't be deleted. ".$this->tecnologia->errMsg;
                        return false;
                    }
                    ...
                    if($tech=="sip"){
                        if(!$this->tecnologia->deletefromDB($result["iperfexweb_device"])){
                            $this->errMsg="Device $device can not be deleted. ".$this->tecnologia->errMsg;
                            return false;
                        }
                    }
    ...
    function createDialPlanLocalExtension(&$arrFromInt){
            ...
            if(is_array($result)){
                foreach($result as $value){
                    ...
                    if($value["rt"]!=0 && isset($value["rt"])){
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_setvar($this->code."_RINGTIMER",$value["rt"]),1);
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_macro($this->code.'-exten-vm',$voicemail.",".$exten));
                    }else
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_macro($this->code.'-exten-vm',$voicemail.",".$exten),1);
                        
                    $arrExtensionLocal[] = new iperfexExtensions($exten, new ext_set('__PICKUPMARK',''),"n",'dest');

                    //if($voicemail != "novm") {
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_goto('1','vmret'));
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_hint($exten,$this->domain),"hint");
                        $arrExtensionLocal[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_macro($this->code.'-vm',$voicemail.',DIRECTDIAL,${IVR_RETVM}'),1);
                        $arrExtensionLocal[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_goto('1','vmret'));
                        $arrExtensionLocal[] = new iperfexExtensions("vmb".$exten,new ext_macro($this->code.'-vm',$voicemail.',BUSY,${IVR_RETVM}'),1);
                        $arrExtensionLocal[] = new iperfexExtensions("vmb".$exten,new ext_goto('1','vmret'));
                        $arrExtensionLocal[] = new iperfexExtensions('vmu'.$exten,new ext_macro($this->code.'-vm',$voicemail.',NOANSWER,${IVR_RETVM}'),1);
                        $arrExtensionLocal[] = new iperfexExtensions('vmu'.$exten,new ext_goto('1','vmret'));
                        $arrExtensionLocal[] = new iperfexExtensions('vms'.$exten,new ext_macro($this->code.'-vm',$voicemail.',NOMESSAGE,${IVR_RETVM}'),1);
                        $arrExtensionLocal[] = new iperfexExtensions('vms'.$exten,new ext_goto('1','vmret'));
                    /*} else {
                        $arrExtensionLocal[] = new iperfexExtensions($exten,new ext_goto('1','return','${IVR_CONTEXT}'));
                        $arrExtensionLocal[]= new iperfexExtensions($exten,new ext_hint($exten,$this->domain),"hint");
                    }*/

                    if(isset($value["alias"]) && $value["alias"]!="")
                        $arrExtensionLocal[] = new iperfexExtensions($value["alias"],new ext_goto('1',$exten),1);
                    
                    // creamos un contexto para que la extensiones locales esten incluidas dentro del ivr
                    $arrExtensionIvr[] = new iperfexExtensions($exten,new ext_execif('$["${BLKVM_OVERRIDE}" != ""]','Noop','Deleting: ${BLKVM_OVERRIDE}: ${DB_DELETE(${BLKVM_OVERRIDE})}'),"1");
                    $arrExtensionIvr[] = new iperfexExtensions($exten,new ext_setvar('__NODEST', ''));
                    $arrExtensionIvr[] = new iperfexExtensions($exten, new ext_goto('1',$exten,$this->code.'-from-did-direct'));
                    ...
                    if($voicemail != "novm") {
                        $arrExtensionIvr[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_execif('$["${BLKVM_OVERRIDE}" != ""]','Noop','Deleting: ${BLKVM_OVERRIDE}: ${DB_DELETE(${BLKVM_OVERRIDE})}'),"1");
                        $arrExtensionIvr[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_setvar('__NODEST', ''));
                        $arrExtensionIvr[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_macro($this->code.'-vm',$voicemail.',DIRECTDIAL,${IVR_RETVM}'));
                        $arrExtensionIvr[] = new iperfexExtensions('${'.$this->code.'_VM_PREFIX}'.$exten,new ext_gotoif('$["${IVR_RETVM}" = "RETURN" & "${IVR_CONTEXT}" != ""]',$this->code.'-ext-local,vmret,playret'));
                    }
                }
            }
            
            $arrExtensionLocal[] = new iperfexExtensions("vmret",new ext_gotoIf('"${IVR_RETVM}" = "RETURN" & "${IVR_CONTEXT}" != ""',"playret"),1);
            $arrExtensionLocal[] = new iperfexExtensions("vmret",new ext_hangup());
            $arrExtensionLocal[] = new iperfexExtensions("vmret",new ext_playback("exited-vm-will-be-transfered&silence/1"),"n","playret");
            $arrExtensionLocal[] = new iperfexExtensions("vmret",new ext_goto("1","return",'${IVR_CONTEXT}'));
            $arrExtensionLocal[] = new iperfexExtensions("h",new ext_macro($this->code."-hangupcall",""),1);
        }

        $contextoLocal=new iperfexContexto($this->code,"ext-local");
        ...
        $contextofromIvr=new iperfexContexto($this->code,"from-did-direct-ivr");
    ...
    function createDialPlanFaxExtension(&$arrFromInt){
        ...
        if($result===false){
            ...
            if(is_array($result)){
                ...
                foreach($result as $value){
                    $exten=$value["exten"];
                    $arrExtensionFax[] = new iperfexExtensions($exten,new ext_noop('Receiving Fax for: '.$value["clid_name"].' ('.$value["clid_number"].'), From: ${CALLERID(all)}'),1);
                    if($value["rt"]!=0 && isset($value["rt"])){
                        $arrExtensionFax[] = new iperfexExtensions($exten,new ext_setvar("RT",$value["rt"]));
                    }
                    $arrExtensionFax[] = new iperfexExtensions($exten,new ext_dial($value["dial"],'${RT}'));
                    $arrExtensionFax[] = new iperfexExtensions($exten,new ext_return());
                    $arrFax[]=new iperfexExtensions("fax",new ext_gosub("1",$exten),$i);
                    $i++;
                }
                $arrFax[]=new iperfexExtensions("fax",new ext_congestion(),$i);
                $arrFax[]=new iperfexExtensions("fax",new ext_macro($this->code."-hangupcall",""),++$i);
                $arrExtensionFax[] = new iperfexExtensions("s",new ext_answer(),1);
                $arrExtensionFax[] = new iperfexExtensions("s",new ext_wait(4));
                $arrExtensionFax[] = new iperfexExtensions("h",new ext_macro($this->code."-hangupcall",""),1);
            }
        }

        $arrExtensionFax=array_merge($arrExtensionFax,$arrFax);
        $contextoFax=new iperfexContexto($this->code,"ext-fax");
