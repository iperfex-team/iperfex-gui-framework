...
/* vim: set expandtab tabstop=4 softtabstop=4 shiftwidth=4:
  ...
  +----------------------------------------------------------------------+
  $Id: iperfexNavigation.class.php,v 1.2 2007/09/07 00:20:03 gcarrillo Exp $ */

define('MENUTAG', 'description');
define('MAX_THEME_LEVEL', 3);

class iperfexNavigationBase
{
...
class iperfexNavigation extends iperfexNavigationBase
{
    ...
    private function includeModule($module)
    {
        ...
        if (!file_exists("{$arrConf['iperfex']}/apps/$module/index.php"))
            return array('data'=>"Error: The module <b>{$arrConf['iperfex']}/apps/$module/index.php</b> could not be found!<br/>");
        ...
        if (file_exists("{$arrConf['iperfex']}/apps/$module/configs/default.conf.php")) {
        ...
        $dirLibs="{$arrConf['iperfex']}/apps/$module/libs";
        if(is_dir($dirLibs)){
        ...
        if($arrCredentials==false)
            return array('data'=>"Error to load User Credentials: {$_SESSION['iperfex_user']}");
        ...
        if(is_dir($directoryScrips)){
            ...
            if($arr_js!=false && count($arr_js)>0){
                for($i=0; $i<count($arr_js); $i++){
                    ...
                    $HEADER_MODULES[] = "<script type='text/javascript' class='header-module-iperfex' src='$dir_script'></script>";
                }
            }
        }
        if(is_dir($directoryCss)){
            ...
            if($arr_css!=false && count($arr_css)>0){
                for($i=0; $i<count($arr_css); $i++){
                    ...
                    $HEADER_MODULES[] = "<link rel='stylesheet' class='header-module-iperfex' href='$dir_css' />";
                }
            }
        }
        return implode("\n", $HEADER_MODULES);
