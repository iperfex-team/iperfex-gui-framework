...
/* vim: set expandtab tabstop=4 softtabstop=4 shiftwidth=4:
  ...
  +----------------------------------------------------------------------+
  $Id: iperfexACL.class.php,v 1.1.1.1 2007/07/06 21:31:55 gcarrillo Exp $ 
  $Id: iperfexACL.class.php,v 3.0 2012/09/01 21:31:55 Rocio Mera rmera@iperfexsanto.com Exp $ */
...
class iperfexACL {
    ...
    var $_DB; // instancia de la clase iperfexDB
    var $errMsg;

    function iperfexACL(&$pDB)
    {
	...
	*/
	function getExtUser($id_user){
		$arr_result2=array();
		$pDB2=new iperfexDB(generarDSNSistema("asteriskuser", "iperfexpbx"));
		if (!preg_match('/^[[:digit:]]+$/', "$id_user")) {
            $this->errMsg = _tr("User ID must be numeric");
		}else{
	...
	*/
     ...
     */
    function changePassword($id_user, $md5_password, $password1)
    {
        $bExito = FALSE;
        if (!preg_match("/^[[:digit:]]+$/", "$id_user")) {
            $this->errMsg = _tr("User ID must be numeric");
            ...
            $this->errMsg = _tr("Password is not a valid MD5 hash");
             ...
             if ($this->errMsg == "") {
				...
				if ($this->_DB->genQuery($sPeticionSQL,array($md5_password,$id_user))) {
					$bExito = TRUE;
                    $string = file_get_contents("/etc/iperfex.conf");
                    $stringArray = explode(PHP_EOL, $string);
                    $itemArray = explode('=', $stringArray[0]);
                    $newString = '';
                    $newString .= $itemArray[0] . "=" . $password1 . PHP_EOL;
                    for($i=1;$i<count($stringArray);$i++)
                    {
                        $newString .= $stringArray[$i] . PHP_EOL;
                    }
                    file_put_contents("/etc/iperfex.conf", $newString);
				} else {
					$this->errMsg = $this->_DB->errMsg;
				}
			}
...
SELECT u.id, u.name, u.username, u.extension, u.fax_extension, e.iperfexweb_device,
...
SQL_ACCOUNTS_INFO;
         ...
         * donde se escribe la columna extension.iperfexweb_device. Esta columna
         * debe contener el valor sin la cadena adjuntada '_dominio.com' para
         * poder funcionar con Kamailio. */
        for ($i = 0; $i < count($result); $i++) {
        	if (substr($result[$i]['iperfexweb_device'], -1 * (strlen($result[$i]['organization_domain']) + 1)) 
                == '_'.$result[$i]['organization_domain']) {
                $result[$i]['iperfexweb_device'] = substr($result[$i]['iperfexweb_device'], 0,
                    strlen($result[$i]['iperfexweb_device']) - strlen($result[$i]['organization_domain']) - 1);
            }
        }
    ...
    function getUserAccountInfo($idUser,$idOrganization){
       $query="SELECT u.id, u.name, u.username, u.extension, u.fax_extension, e.iperfexweb_device, e.alias
