...
/* vim: set expandtab tabstop=4 softtabstop=4 shiftwidth=4:
  ...
  +----------------------------------------------------------------------+
  $Id: iperfexSampler.class.php,v 1.1.1.1 2007/07/06 21:31:55 gcarrillo Exp $ */

class iperfexSampler {

    private $rutaDB;
    private $errMsg;
    private $_db;

    function iperfexSampler()
    {
        global $arrConf;
        $this->rutaDB = $arrConf['iperfex_dsn']['samples'];
        //instanciar clase iperfexDB
        $pDB = new iperfexDB($this->rutaDB);
	    if(!empty($pDB->errMsg)) {
        	echo "$pDB->errMsg <br>";
		}else{
			$this->_db = $pDB;
