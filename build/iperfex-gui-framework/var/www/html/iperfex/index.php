...
function spl_iperfex_class_autoload($sNombreClase)
{
...
spl_autoload_register('spl_iperfex_class_autoload');

// Agregar directorio libs de script a la lista de rutas a buscar para require()

$iperfex="/usr/share/iperfex";
// /usr/share/iperfex/ directorio que contiene las librerias del sistema
//
ini_set('include_path',dirname($_SERVER['SCRIPT_FILENAME']).":$iperfex:".ini_get('include_path'));

include_once "libs/misc.lib.php";
include_once "configs/default.conf.php";
include_once "libs/iperfexDB.class.php";
include_once "libs/iperfexACL.class.php";
include_once "libs/iperfexMenu.class.php";
include_once "libs/iperfexNavigation.class.php";

$arrConf['basePath']=$arrConf['basePath']."/iperfex"; //se cambia la ubicacion del modulo
...
if(isset($_GET['logout']) && $_GET['logout']=='yes') {
    $user = isset($_SESSION['iperfex_user'])?$_SESSION['iperfex_user']:"unknown";
...
// 2) Autentico usuario
if (isset($_SESSION['iperfex_user']) && 
    isset($_SESSION['iperfex_pass']) && 
    $pACL->authenticateUser($_SESSION['iperfex_user'], $_SESSION['iperfex_pass'])) {
    ...
    $_SESSION['iperfex_organization'] = $id_organization;
    ...
    $smarty->assign("ABOUT_ELASTIX_CONTENT", _tr('About Iperfex Content'));
    $smarty->assign("ABOUT_CLOSED", _tr('About Iperfex Closed'));
    ...
    $smarty->assign("ABOUT_ELASTIX", _tr('About Iperfex')." ".$arrConf['iperfex_version']);

    $selectedMenu = getParameter('menu');
    /* El módulo _iperfexutils sirve para contener las utilidades json que
     * atienden requerimientos de varios widgets de la interfaz Iperfex. Todo
     * requerimiento nuevo que no sea un módulo debe de agregarse aquí */
    // TODO: agregar manera de rutear _iperfexutils a través de iperfexNavigation
    if (!is_null($selectedMenu) && $selectedMenu == '_iperfexutils' && 
        file_exists("$iperfex/apps/_iperfexutils/index.php")) {
        
        // Cargar las configuraciones para el módulo elegido
        if (file_exists("$iperfex/apps/_iperfexutils/configs/default.conf.php")) {
            require_once "apps/_iperfexutils/configs/default.conf.php";
        ...
        require_once "apps/_iperfexutils/index.php";
        echo _moduleContent($smarty, $selectedMenu);
        return;
    }

    // Inicializa el objeto iperfexsanto navigation
    $oPn = new iperfexNavigation($arrMenuFiltered, $smarty, $selectedMenu);
    ...
    if(isset($rawmode) && $rawmode=='yes'){
        ...
        $oPn = new iperfexNavigation(array(), $smarty);
