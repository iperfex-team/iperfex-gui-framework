$(document).ready(function(){
    ...
    var channels = parseInt($("input[name=channels]").attr('max')) + parseInt($("input[name=channels]").val());

    $("input[name=channels]").TouchSpin({
        min: 0,
        max: channels,
        step: 1,
        boostat: 5,
...
function change_state(){
    ...
    if(orgs.length!=0){
        ...
        $('.neo-modal-iperfex-popup-close').css("display","none");
        ...
        request("index.php", arrAction, false,
            function(arrData,statusResponse,error){
                $('.neo-modal-iperfex-popup-close').css("display","block");
