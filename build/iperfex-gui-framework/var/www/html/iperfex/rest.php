...
require_once "$documentRoot/libs/iperfexJSON.class.php";
require_once "$documentRoot/libs/iperfexACL.class.php";
...
$json = new iperfexJSON();
header('Content-Type: application/json');

/***********************User authentication********************************/
$pACL = new iperfexACL($arrConf['iperfex_dsn']['acl']);
...
session_start();
if (isset($_SESSION['iperfex_user']) && isset($_SESSION['iperfex_pass'])) {
    $auth_user = $_SESSION['iperfex_user'];
    $auth_md5pass = $_SESSION['iperfex_pass'];    
    $_SERVER['PHP_AUTH_USER'] = $_SESSION['iperfex_user'];
} elseif (isset($_SERVER['PHP_AUTH_USER']) && $_SERVER['PHP_AUTH_USER'] != '') {
    $auth_user = $_SERVER['PHP_AUTH_USER'];
    $auth_md5pass = md5($_SERVER['PHP_AUTH_PW']);
} else {
    header('HTTP/1.1 401 Unauthorized');
    header('WWW-Authenticate: Basic realm="IperfexWebService"');
    $json->set_status("ERROR");
