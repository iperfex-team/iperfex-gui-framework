var iperfex_flag_changed_profile = false;
...
$(document).ready(function() {
	...
	$(this).on('change', '#languageProfile', function() {
		$.post('index.php', {
			...
			iperfex_flag_changed_profile = true;
		});
	});
	...
	$(this).on('click', '#iperfex_link_change_passwd', function() {
		$("#iperfex_data_change_passwd").toggle();
	});
	
	// Recargar la página si un cambio afecta la totalidad de la interfaz
	$(this).on('click', '.iperfex_close_popup_profile', function() {
		if (iperfex_flag_changed_profile) shutdownSIPClient(function() {
			location.reload();
		});
	})
	...
	$(this).on('click', '#currentPasswordProfile', function() {
		$('#currentPasswordProfile').keyup(function() {
			var targets = $('#newPasswordProfile, #repeatPasswordProfile, #iperfex_save_change_passwd');
	...
	$(this).on('click', '#iperfex_save_change_passwd', function() {
		$.post('index.php', {
			menu:			'_iperfexutils',
			action:			'changePasswordIperfex',
			rawmode:		'yes',
			...
			$("#iperfex_data_change_passwd").hide();
		});
	});
});
