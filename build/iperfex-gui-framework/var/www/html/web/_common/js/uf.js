$(document).ready(function(){
    $(this).on('click','.iperfex-msg-area-close',function(e){
        $("#iperfex_msg_area").slideUp();
    });
                            
    $(window).resize(function(){
    	var main_content_div = $('#main_content_iperfex'); //div que contiene lo que cada modulo tiene
    	var rightdiv = $('#rightdiv'); //panel lateral en donde aparece el chat
        var w = $(window).width();
        var tmpSize=0;
        if(w>=500){
            if(rightdiv.is(':hidden') == false){ //esta abierto
                $("#iperfex_chat_space").css("right","200px");
                tmpSize= tmpSize + 180;
            }
            tmpSize = w - tmpSize;
            main_content_div.css("width",tmpSize+"px");
            $("#iperfex_chat_space").show(1);
        }else{
            if(rightdiv.is(':hidden') == false){ //esta abierto
                $("#iperfex_chat_space").hide(1,function(){
                    $("#iperfex_chat_space").css("right","15px");
                });
            }else
                $("#iperfex_chat_space").css("right","15px");
    ...
    $(this).on('click','#icn_disp2',function(e){
        ...
        if( rightdiv.is(':hidden') ){ //estaba oculto y lo abrimos
            ...
            if(w>=500){
                //es necesario modificar la el margin right del espacio del chat
                $("#iperfex_chat_space").css("right",15+"px");
                //si esta abierto lo coultamos y modificamos el tamaño de la pantalla
                tmpSize = w;
                main_content_div.css("width",tmpSize+"px");
            }else{
                $("#iperfex_chat_space").show(10);
            }
            rightdiv.hide(10);
            $("#iperfex_im_list_contacts").css('height','');
            adjustTabChatToWindow(w);
        }
    });
    ...
    $(this).on('click', '.iperfex-display-dialog-show-profile', function() {
    	$.get('index.php', {
    		menu:	'_iperfexutils',
    		...
    		$("#iperfex_popup_content").html(response.message);
    		$('#iperfex_general_popup').modal({show: true});
    	});
    });
    
    //despliega en efecto slider el menu oculto, en tamño < 480px;
    $(this).on('click','#iperfex-navbar-min',function(){
        if ($("#iperfex-slide-menu-mini").is(":hidden") ) {
            $("#iperfex-slide-menu-mini").slideDown("slow");
            setTimeout(function() { $("#iperfex-slide-menu-mini").css('overflow','visible'); }, 600);
            $("#rightdiv").animate({
                top: "88px"
            }, 600 );
        } else {
            $("#iperfex-slide-menu-mini").slideUp("slow");
            $("#rightdiv").animate({
                top: "55px"
            }, 600 );
        }
    ...
    $('#iperfex-chk-show-offline-contacts').click(updateContactVisibility);
...
function setupChatWindowHandlers(doc)
{
	...
	$(doc).on('click', '.iperfex_li_contact', function() {
        // Ahorrar espacio para caso de ventana estrecha
        if ($(window).width() < 500) $('#rightdiv').hide(10);
        
        startChatUser($(this).data('uri'), 'sent')
        	.find('.iperfex_text_area_chat > textarea').focus();
        $("#iperfex_chat_space").show(10);
    });

	// Acciones para controlar las ventanas de chat
    $(doc).on('click', '.iperfex_close_chat', function() {
        // Cerrar la ventana del chat (realmente la oculta, pero el div sigue presente)

        $(this).parents(".iperfex_tab_chat").removeClass('iperfex_chat_active').addClass('iperfex_chat_close');
        //debemos comprobar si ahi pestañas minimizadas por falta de espacio
        //si existen entonces abrimos la ultima pestaña
        var chatMIn=$("#iperfex_chat_space_tabs > .iperfex_chat_min").last();
        if(chatMIn!=='undefined'){
            chatMIn.removeClass('iperfex_chat_min').addClass('iperfex_chat_active');
            removeElxUserNotifyChatMini(chatMIn);
        }
    });
    $(doc).on('click', '.iperfex_min_chat', function() {
        // Minimizar la ventana de chat    	
        $(this).removeClass("glyphicon-minus iperfex_min_chat").addClass("glyphicon-resize-vertical iperfex_max_chat");
        $(this).parents(".iperfex_header_tab_chat").next('.iperfex_body_tab_chat').css('display','none');
    });
    $(doc).on('click', '.iperfex_max_chat', function() {
        // Restaurar la ventana de chat    	
        $(this).removeClass("glyphicon-resize-vertical iperfex_max_chat").addClass("glyphicon-minus iperfex_min_chat");
        $(this).parents(".iperfex_header_tab_chat").next('.iperfex_body_tab_chat').css('display','block');
	});
	$(doc).on('click', 'div.iperfex_header2_tab_chat > span.glyphicon-envelope', function() {
		// Envío de correo al usuario del chat
		iperfex_newEmail($(this).parents('.iperfex_tab_chat').data('alias'));
	});
	$(doc).on('click', 'div.iperfex_header2_tab_chat > span.glyphicon-print', function() {
		// Envío de fax al usuario del chat
		showSendFax($(this).parents('.iperfex_tab_chat').data('alias'));
	});
	$(doc).on('click', 'div.iperfex_header2_tab_chat > span.glyphicon-earphone', function() {
		...
		var contactInfo = lookupSIPRoster($(this).parents('.iperfex_tab_chat').data('uri'));
		showCallWindow(
	...
	$(doc).on("keydown",".iperfex_chat_input", function( event ) {
        // Ignore TAB and ESC.
        if (event.which == 9 || event.which == 27) {
            ...
            var iperfex_txt_chat=$(this).val();
            var iperfex_tab_chat=$(this).parents('.iperfex_tab_chat:first');
            
            $(this).val('');
            sendMessage(iperfex_txt_chat, iperfex_tab_chat.data('uri'));
	...
	$(doc).on("click",".iperfex_tab_chat", function( event ) {
	    $(this).children('.iperfex_header_tab_chat').removeClass('iperfex_blink_chat');
	    $(this).find('.iperfex_text_area_chat > textarea').focus();
	});
	
	//motificaciones en pestañas de chat minimizadas por falta de espacio
	$('#iperfex_notify_min_chat_box').on("click",function(event){
	    var hidMinList=$('#iperfex_hide_min_list').val();
	    if(hidMinList=='yes'){
	        //se deben ocultar la lista de las conversaciones minimizadas por falta de espacio
	        $("#iperfex_notify_min_chat_box").removeClass('iperfex_notify_min_chat_box_act');
	        $('#iperfex_hide_min_list').val('no');
	        $("#iperfex_list_min_chat").css('visibility','hidden');
	    }else{
	        ...
	        $("#iperfex_notify_min_chat_box").addClass('iperfex_notify_min_chat_box_act');
	        $('#iperfex_hide_min_list').val('yes');
	        var offElement=$("#iperfex_notify_min_chat_box").offset();
	        var widthList = $("#iperfex_list_min_chat > div > .iperfex_list_min_chat_ul").width();
	        if((offElement.left-40)>(widthList)){
	            //existe suficiente espacio para mostrar la lista 
	            $("#iperfex_list_min_chat").css('right','0px');
	            $("#iperfex_list_min_chat").css('left','');
	        }else{
	            //no existe suficiente espacio para mostrar la lista 
	            $("#iperfex_list_min_chat").css('left','0px');
	            $("#iperfex_list_min_chat").css('right','');
	        }
	        $("#iperfex_list_min_chat").css('visibility','visible');
	    }
	});
	$(doc).on('click', '.iperfex_min_name', function(event) {
	    $(this).children(".iperfex_min_chat_num").css('visibility','hidden');
	    //se deben ocultar la lista de las conversaciones minimizadas por falta de espacio
	    $("#iperfex_notify_min_chat_box").removeClass('iperfex_notify_min_chat_box_act');
	    $('#iperfex_hide_min_list').val('no');
	    $("#iperfex_list_min_chat").css('visibility','hidden');
	    var uri = $(this).parents('.iperfex_list_min_chat_li:first').data('uri');
	    var iperfex_tab_chat = startChatUser(uri, 'sent');
	    iperfex_tab_chat.find('.iperfex_text_area_chat > textarea').focus();
	    iperfex_tab_chat.find('.iperfex_tab_tittle_icon > span:first').removeClass("glyphicon-resize-vertical iperfex_max_chat").addClass("glyphicon-minus iperfex_min_chat");
	    iperfex_tab_chat.find('.iperfex_body_tab_chat').css('display','block');
	});
	$(doc).on('click','.iperfex_min_remove',function(event){
	    var liIcon = $(this).parents('.iperfex_list_min_chat_li:first');
	    var uri = liIcon.data('uri');
	    liIcon.remove();
	    var tabChat = getTabElxChat(uri);
	    tabChat.removeClass('iperfex_chat_min').addClass('iperfex_chat_close');
...
{
	$(doc).on('click', '#iperfex_presence_online', function() {
		if (sp != null) sp.setPresenceStatus('Online', []);
	});
	$(doc).on('click', '#iperfex_presence_away', function() {
		if (sp != null) sp.setPresenceStatus('Away', ['away']);
	});
	$(doc).on('click', '#iperfex_presence_meeting', function() {
		if (sp != null) sp.setPresenceStatus('On a meeting', ['meeting']);
	});
	$(doc).on('click', '#iperfex_presence_busy', function() {
		if (sp != null) sp.setPresenceStatus('Busy (DND)', ['busy']);
	});
	$(doc).on('click', '#iperfex_presence_offline', function() {
...
{
    $(doc).on('click', '.iperfex-display-dialog-show-sendfax', function() {
    	showSendFax(false);
    })    
    
    //oculta o muestra la opcion de subir archivo para la opción "sendfax"
    $(doc).on('click','#iperfex-chk-attachment-file',function(){
        if($(this).is(':checked')) {
            $("#iperfex-body-fax-label").removeClass("visible").addClass("oculto");
            $("#iperfex-body-fax-content").removeClass("visible").addClass("oculto");
            $("#iperfex-attached-fax-file").removeClass("oculto").addClass("visible");
            $("#iperfex-notice-fax-file").removeClass("oculto").addClass("visible");
            $("textarea[name='faxContent']").val("")
        }else{
            $("#iperfex-body-fax-label").removeClass("oculto").addClass("visible");
            $("#iperfex-body-fax-content").removeClass("oculto").addClass("visible");
            $("#iperfex-attached-fax-file").removeClass("visible").addClass("oculto");
            $("#iperfex-notice-fax-file").removeClass("visible").addClass("oculto");
        }
    });
}
...
function scrollContentModule(){
	if( $('.iperfex-modules-content').length )
    {
        var height_browser = $(window).height();
        var offElement=$(".iperfex-modules-content").offset();
        $(".iperfex-modules-content").css("height",height_browser-offElement.top +"px");
    }
}
function iperfexTitleAlert(message){
...
function adjustHeightElxListUser(){
    ...
    $("#iperfex_im_list_contacts").css('height',max_h+"px");
}
/*
function changeModuleUF(moduleName){
    ...
    showIperfexUFStatusBar('Loading Module...');
    ...
    request("index.php",arrAction,false,
        function(arrData,statusResponse,error){
            ...
            if(error!=''){
                ...
                var content="<input type='hidden' id='iperfex_framework_module_id' value='"+moduleName+"' />";
...
*/
function showIperfexUFStatusBar(msgbar){
    $("#notify_change_iperfex").css('display','block');
    if(msgbar){
        $(".progress-bar-iperfex").html(msgbar);
    }else{
        $(".progress-bar-iperfex").html('Loading..');
    }
}
function hideIperfexUFStatusBar(){
    $("#notify_change_iperfex").css('display','none');
}
function showElxUFMsgBar(status,msgbar){
    if(status=='error'){
        $("#iperfex_msg_area_text").removeClass("alert-success").addClass("alert-danger");
    }else{
        $("#iperfex_msg_area_text").removeClass("alert-danger").addClass("alert-success");
    }
    $("#iperfex_msg_area_text").html(msgbar);
    $("#iperfex_msg_area").slideDown(); 
}
...
function setupSIPClient()
{
    ...
    $(".iperfex-content-photo").css('border-color', 'gray');
    ...
    $.get('index.php', {
        ...
        if (response.error != '') {
            ...
            ua = new SIP.UA({
                uri:                response.message.iperfexuser_username,
            ...
            ua.on('message', function (e) {
                ...
                var iperfex_txt_chat = e.body;

                receiveMessage(uri2, e.contentType, iperfex_txt_chat);
    ...
    $.get('index.php', {
        ...
        if (response.error != '') {
            ...
            for (var i = 0; i < response.message.length; i++) {
                $("#iperfex_ul_list_contacts").append(
                	createDivContact(
...
function updateContactVisibility()
{
    ...
    var mostrarTodos = $('#iperfex-chk-show-offline-contacts').is(':checked');

    // Ocultar todos los contactos, y volver a mostrar sólo coincidencias 
    $("#iperfex_ul_list_contacts > li.iperfex_li_contact").hide().each(function() {
...
{
	var liContact = $('#iperfex_template_contact_status > li').clone()
		.addClass('iperfex_li_contact')
	...
	liContact.find('.iperfex_im_name_user').text(display_name);
...
function lookupSIPRoster(keyval)
{
	...
	for (var i = 0; i < dataKeys.length; i++) {
		liContact = $('ul#iperfex_ul_list_contacts > li[data-' + dataKeys[i] + '="' + keyval + '"]');
...
function receiveMessage(urioralias, content_type, msg_txt)
{
    ...
    var iperfex_tab_chat = startChatUser(urioralias, 'receive');
    if (!iperfex_tab_chat.hasClass('iperfex_chat_min')){
        if (!iperfex_tab_chat.find('.iperfex_text_area_chat > textarea').is(':focus')){
            //añadimos clase que torna header anaranjado para indicar que llego nuevo mensaje
            iperfex_tab_chat.children('.iperfex_header_tab_chat').addClass('iperfex_blink_chat');
        }
    }
    ...
    if (content_type == 'application/im-iscomposing+xml') {
    	...
    	setComposingStateElxChatTab(iperfex_tab_chat, state);
    } else {
        addMessageElxChatTab(iperfex_tab_chat, 'in', msg_txt);
    }
}
...
{    
    var iperfex_tab_chat = getTabElxChat(uri);    
    addMessageElxChatTab(iperfex_tab_chat, 'out', msg_txt);
    ...
    ua.message(uri, msg_txt).on('failed', function (response, cause) {
        ...
        if (iperfex_tab_chat != false) {
            iperfex_tab_chat.find(".iperfex_content_chat").find("div").last().css("color","red");
            addMessageElxChatTab(iperfex_tab_chat, 'in', $('<span style="color: red;"></span>').text(error_msg));
        } else {
            alert(error_msg);
        }
    });
...
{
	var chatTab = $("#iperfex_chat_space_tabs > .iperfex_tab_chat[data-uri='" + uri + "'] :first");
...
function startChatUser(urioralias, action)
{
    ...
    var iperfex_tab_chat = getTabElxChat(contactInfo.uri);

    if (!iperfex_tab_chat) {
    	// Ventana para este usuario no existe, se debe de crear una nueva
        
        var content = $('#iperfex_template_tab_chat > .iperfex_tab_chat').clone()
        	.attr('data-uri', contactInfo.uri);
        content.find('.iperfex_tab_chat_name_span').text(contactInfo.name);
    
        // Se agrega el nuevo chat a la lista de ventanas de chat
        $("#iperfex_chat_space_tabs").prepend(content);
        iperfex_tab_chat = $("#iperfex_chat_space_tabs > .iperfex_tab_chat:first");
        
        /* Llegado a este punto, la situación es igual que si se hubiese 
         * encontrado anteriormente la ventana, en estado minimizado. */
    }

    if (!iperfex_tab_chat.hasClass('iperfex_chat_active')) {
        ...
        if (can_add_chat) {
            ...
            if(iperfex_tab_chat.hasClass('iperfex_chat_close')){
                ...
                iperfex_tab_chat.removeClass('iperfex_chat_close').addClass('iperfex_chat_active');
                removeElxUserNotifyChatMini(iperfex_tab_chat);
            } else if(iperfex_tab_chat.hasClass('iperfex_chat_min')){
                iperfex_tab_chat.removeClass('iperfex_chat_min').addClass('iperfex_chat_active');
                removeElxUserNotifyChatMini(iperfex_tab_chat);
            }
        }
    }
    ...
    if (action == 'receive' && !can_add_chat) {
        addElxUserNotifyChatMini(iperfex_tab_chat);
        iperfex_tab_chat.removeClass('iperfex_chat_close').addClass('iperfex_chat_min');
        $("#iperfex_notify_min_chat_box").addClass('iperfex_notify_min_chat_box_act');
        
        // Hacer visible el asterisco al lado del item de chat minimizado que recibe el mensaje
        $('#iperfex_list_min_chat > div > .iperfex_list_min_chat_ul > .iperfex_list_min_chat_li[data-uri="'
        		+ contactInfo.uri + '"] .iperfex_min_name > .iperfex_min_chat_num')
        	.css('visibility', 'inherit');
    }

    return iperfex_tab_chat;
}

/**
 * Esta función verifica si se puede acomodar una nueva pestaña de chat en el 
 * ancho de pantalla indicado por iperfex_w. Si se alcanzó el máximo, en el caso de
 * sent, es posible minimizar otra pestaña para acomodar la nueva.
 * 
 * @param	iperfex_w	Ancho en pixeles de la pantalla
 ...
 */
function resizeElxChatTab(iperfex_w, action)
{
	var max_tab = getMaxNumTabChat(iperfex_w);
    var num_act_chat = $("#iperfex_chat_space_tabs > .iperfex_chat_active").size();
    ...
    var iperfexTabChatToMin = $("#iperfex_chat_space_tabs > .iperfex_chat_active").first();
    iperfexTabChatToMin.removeClass('iperfex_chat_active').addClass('iperfex_chat_min');
    addElxUserNotifyChatMini(iperfexTabChatToMin);
    return true;
}
 ...
 * 
 * @param iperfex_w		Ancho de pantalla para usar en cálculo
 * @returns {Number}
 */
function getMaxNumTabChat(iperfex_w)
{
	// Restar del ancho disponible el ancho de la lista de chat
	if (!$("#rightdiv").is(':hidden')) iperfex_w -= 180;

	if (iperfex_w < 320) return 1;
	if (iperfex_w >= 1240) return 4; // Más allá sólo se permitirán 4 pestañas abiertas
	return Math.floor((iperfex_w - 90) / 230);	// Cada pestaña reserva 230px
}
...
function addMessageElxChatTab(chatTab, direction, message)
{
    var send_name = (direction == 'out') 
        ? 'me'
        : chatTab.find('.iperfex_tab_chat_name > .iperfex_tab_chat_name_span').text();
    ...
    if (direction != 'out') iperfexTitleAlert('New Message ' + send_name);
    
    var messagediv = $('<div></div>');
    if (typeof message == 'string') 
    	messagediv.text(message);
    else messagediv.append(message);
    chatTab.find('.iperfex_body_tab_chat:first .iperfex_content_chat:first')
    	.append(messagediv.prepend($('<b></b>').text(send_name + ': ')))
    	.scrollTop(1e4);
    if (direction != 'out') {
    	chatTab.find('.iperfex_body_tab_chat:first .iperfex_content_chat:first div.iperfex_chat_composing').remove();
    }
}
...
function setComposingStateElxChatTab(chatTab, state)
{
	...
	var target = chatTab.find('.iperfex_body_tab_chat:first .iperfex_content_chat:first');
	target.find('div.iperfex_chat_composing').remove();
	if (state == 'active') {
		var messagediv = $('<div class="iperfex_chat_composing"></div>');
		messagediv.text('composing...');
		target.append(messagediv).scrollTop(1e4);
	}
}

function addElxUserNotifyChatMini(iperfexTabChatToMin)
{
	var uri = iperfexTabChatToMin.data('uri');
	var list_min_chat = $('#iperfex_list_min_chat > div > ul.iperfex_list_min_chat_ul');
	if (list_min_chat.find('li.iperfex_list_min_chat_li[data-uri="' + uri + '"]').size() > 0)
		return;
	
	var contactInfo = lookupSIPRoster(uri);
	if (contactInfo.uri == null) {
		contactInfo.name = iperfexTabChatToMin.find('.iperfex_tab_chat_name > .iperfex_tab_chat_name_span').html();
	}
	
	// Clonar plantilla y asignar data-uri y nombre
	var iperfex_chat_user = $('#iperfex_template_min_chat_ul > li.iperfex_list_min_chat_li')
		.clone().attr('data-uri', uri);
	iperfex_chat_user.find('span.iperfex_min_chat_name').text(contactInfo.name);
	list_min_chat.prepend(iperfex_chat_user);
	
	// Actualizar número de chats minimizados
	$("#iperfex_num_mim_chat").text(list_min_chat.find('li.iperfex_list_min_chat_li').size());
	
    // Si no se estaba mostrando el chat con las notificaciones lo mostramos
    $("#iperfex_notify_min_chat").removeClass('iperfex_nodisplay');
}

function removeElxUserNotifyChatMini(iperfexTabChatToMin)
{
    var uri = iperfexTabChatToMin.data('uri');
    var list_min_chat = $('#iperfex_list_min_chat > div > ul.iperfex_list_min_chat_ul');
    list_min_chat.find('li.iperfex_list_min_chat_li[data-uri="' + uri + '"]').remove();
    var numMinchat = list_min_chat.find('li.iperfex_list_min_chat_li').size();

    if (numMinchat == 0) {
        //ocultamos el div con las notificaciones
        $("#iperfex_notify_min_chat").addClass('iperfex_nodisplay');
        $("#iperfex_notify_min_chat_box").removeClass('iperfex_notify_min_chat_box_act');
        $('#iperfex_hide_min_list').val('no');
        $("#iperfex_list_min_chat").css('visibility','hidden');
    } else {
        //actualizazamos la informacion del numero de chat abiertos minimizados
        $("#iperfex_num_mim_chat").text(numMinchat);
    }
}

function adjustTabChatToWindow(iperfexw){ 
    //contralamos el número de pestañas activas abiertas de acuerdo al tamamaño de la pantalla
    var max_tab=getMaxNumTabChat(iperfexw); 
...
function iperfexGridData(moduleName, action, arrFilter, page){
    var currentNumPage=$("#iperfexGridNumPage").val();
    ...
    request("index.php", arrAction, false,
        function(arrData,statusResponse,error){
            if (error != '' ){
                ...
                $("#iperfex_data_grid > table > tbody").html(content);
                ...
                $("#iperfexGridNumPage").val(arrData['numPage']);
                $("#iperfexGridCurrent").val(page);
                
                var options = {
                    currentPage: page,
                    totalPages: $("#iperfexGridNumPage").val(),
                    }
                
                $('#iperfex_pager').bootstrapPaginator(options); 
    ...
    request("index.php", arrAction, false,
        function(arrData,statusResponse,error){
            if(error != ''){
                ...
                $("#iperfex_popup_content").html(arrData);
                var options = {
                    show: true
                    }
                $('#iperfex_general_popup').modal(options);
                formSendFax();
            }
        }
    );       
...
function sendNewFax(){
    ...
    if($('#iperfex-chk-attachment-file').is(':checked')) {
    ...
    request("index.php", arrAction, false,
        function(arrData,statusResponse,error){
            if (error != '' ){
                ...
                alert(arrData);
                $('#iperfex_general_popup').modal('hide');
            }
    });      
}
...
function iperfex_newEmail(alias){
    var arrAction = new Array();
    arrAction["menu"]="home";
    ...
    request("index.php", arrAction, false,
        function(arrData,statusResponse,error){
            if(error != ''){
                ...
                $("#iperfex_popup_content").html(arrData['modulo']);
                $("#iperfex-compose-email").addClass("modal-content");
                $("#iperfex-compose-email").prepend("<div class='modal-header'><button type='button' class='close' data-dismiss='modal' aria-hidden='true'>&times;</button><h3 id='myModalLabel'>Send Mail/Enviar Mail</h3></div>");
                $("#iperfex-compose-email").append("<div class='modal-footer'><button type='button' class='btn btn-primary' id='iperfex_attachButton'>Attach<input type='file' name='attachFileButton' id='attachFileButton'></button><button type='button' class='btn btn-primary' onclick='composeEmail(\"popup\")'>Send</button></div>");    
                emailAttachFile();
                var options = {
                    show: true
                    }
...
{
	var divCall = $('#iperfex_template_videocall > div.modal-content').clone();	
	$("#iperfex_popup_content").html(divCall);
	divCall = $("#iperfex_popup_content > div.modal-content");
	...
	divCall.find('#iperfex_videocall_dial').on('click', startOutgoingCall);
	divCall.find('#iperfex_videocall_reject').on('click', function() {
		if (sess != null) sess.reject();
	});
	divCall.find('#iperfex_videocall_cancel').on('click', function() {
		if (sess != null) sess.cancel();
	});
	divCall.find('#iperfex_videocall_accept').on('click', function() {
		if (sess != null) sess.accept(getVideoCallMedia(divCall));
	});
	divCall.find('#iperfex_videocall_hangup').on('click', function() {
	...
	if (incoming_sess != null) {
		divCall.find('div.iperfex_video_call_buttons > button').hide();
		divCall.find('#iperfex_videocall_accept').show();
		divCall.find('#iperfex_videocall_reject').show();
		divCall.find('input[name="iperfex_videocall_dialstring"]').attr('disabled', true);
		
		divCall.find('input[name="iperfex_videocall_dialstring"]').val(dialstring);
		sess = incoming_sess;
...
{
	divCall.find('div.iperfex_video_callstatus')
		.attr('role', 'alert')
		.removeClass('alert-success alert-info alert-warning alert-danger')
		.addClass('alert')
		.addClass(extraclass)
		.text(msgtext).show();
	if (hide) divCall.find('div.iperfex_video_callstatus').delay(5000).fadeOut(500);
}
...
{
	var divCall = $("#iperfex_popup_content > div.modal-content");
	var videoRow = divCall.find('.iperfex_video_row');
	var dialInput = divCall.find('input[name="iperfex_videocall_dialstring"]');
	...
	divCall.find('#iperfex_video_remote').attr('poster', 'index.php?' + $.param(videoPosterParams));
	videoPosterParams.dialstring = ua.configuration.authorizationUser;
	divCall.find('#iperfex_video_local').attr('poster', 'index.php?' + $.param(videoPosterParams));
	
	// Desactivar el ingreso de dialstring mientras haya llamada en progreso
	divCall.find('div.iperfex_video_call_buttons > button').hide();
	divCall.find('#iperfex_videocall_cancel').show();
...
function getVideoCallMedia(divCall)
{
	return {
		media: {
			...
			render: {
				remote: {
					video: divCall.find('#iperfex_video_remote').get(0),
					audio: divCall.find('#iperfex_audio_remote').get(0)
				},
				local: {
					video: divCall.find('#iperfex_video_local').get(0)
				}
			}
		}
	};
...
function transitionCall_DialReady(divCall)
{
	...
	videoRow.hide();
	divCall.find('div.iperfex_video_call_buttons > button').hide();
	divCall.find('#iperfex_videocall_dial').show();
...
function transitionCall_Accepted(divCall)
{
	...
	videoRow.show();
	divCall.find('div.iperfex_video_call_buttons > button').hide();
	divCall.find('#iperfex_videocall_hangup').show();
	dialInput.attr('disabled', true);
}
...
function SIPPresence(ua)
{
	...
	this.publishPresence = function () {
		...
		$(".iperfex_li_contact").each(function(i, v) {
		...
		if (this.publishRequest == null) {
			...
			this.publishRequest.on('accepted', function (response, cause) {
				if (response.getHeader('Expires') != "0") {
					...
					// Elegir el color a mostrar según notas y actividades
					$(".iperfex-content-photo").css('border-color', this.presentity.suggestStateColor());					
				} else {
					this.publishRequest = null;
					this.presenceETag = null;
					$.post('index.php', {
	...
	*/
	...
	this._updateContactStatus = function(contact, newColor, newNote) {
		var liContact = $(".iperfex_li_contact[data-uri='" + contact + "']");
