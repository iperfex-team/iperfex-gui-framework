	...
	request("index.php",arrAction,false,
		function(arrData,statusResponse,error)
		{
			...
			if(statusResponse == "OK"){
				...
				$("#neo-sticky-note-text-edit").addClass("neo-display-none");
				$("#neo-sticky-note").data("neo-sticky-note-status", "hidden");
				var themeName = $('#iperfex_theme_name').val();
				if(themeName == "iperfexneo"){
					if(arrAction['description'] != ""){
						var imgName = "web/themes/iperfexneo/images/tab_notes_on.png";
						$('#togglestickynote1').attr('src',imgName);
					}else{
						var imgName = "web/themes/iperfexneo/images/tab_notes.png";
						$('#togglestickynote1').attr('src',imgName);
					}
				}
			}else{
