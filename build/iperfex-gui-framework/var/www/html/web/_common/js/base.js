...
var modal_iperfex_popup_shown = false;

function ShowModalPopUP(title, width, height, html){

    $('.neo-modal-iperfex-popup-content').html(html);
    $('.neo-modal-iperfex-popup-title').text(title);

    var maskHeight = $(document).height();
    var maskWidth = $(window).width();

    $('.neo-modal-iperfex-popup-blockmask').css({'width':maskWidth,'height':maskHeight});

    $('.neo-modal-iperfex-popup-blockmask').fadeIn(600);
    $('.neo-modal-iperfex-popup-blockmask').fadeTo("fast",0.8);
    ...
    if(top<0){
        top=10;
        $('.neo-modal-iperfex-popup-box').css({'height':"auto", 'bottom':10});
        $('.neo-modal-iperfex-popup-content').css({'overflow-y':"auto", 'overflow-x':"visible", 'bottom':20, 'position': "absolute", 'top':40, 'width':"93%"});

    }else{
        $('.neo-modal-iperfex-popup-box').height(height);
    }
    $('.neo-modal-iperfex-popup-box').width(width);
    $('.neo-modal-iperfex-popup-box').css('top',  top);
    $('.neo-modal-iperfex-popup-box').css('left', winW/2-width/2);
    $('.neo-modal-iperfex-popup-box').fadeIn(2000);

    modal_iperfex_popup_shown = true;

    $('.neo-modal-iperfex-popup-close').click(function() {
        hideModalPopUP();
    });
}
...
{
    $('.neo-modal-iperfex-popup-box').fadeOut(10);
    $('.neo-modal-iperfex-popup-blockmask').fadeOut(20);
    $('.neo-modal-iperfex-popup-content').html("");

    modal_iperfex_popup_shown = false;
}

function showPopupIperfex(id, title, width, height){
...
function setAdminPassword(){
    ...
    var html =
        "<table class='tabForm' style='font-size: 16px;' width='100%' >" +
            ...
            "</tr>" +
            "<tr class='letra12' >" +
                "<td align='center'  colspan='2'><input type='button' id='iperfexSendChanPass' name='sendChanPss' value='"+btnChange+"' onclick='saveNewPasswordIperfex()' /><img id='iperfexImgSavingPass' style='display:none' src='"+webCommon+"images/loading.gif' /></td>" +
            "</tr>" +
        "</table>";
    ShowModalPopUP(title,width,height,html);
}
function saveNewPasswordIperfex(){
    var arrAction = new Array();
    ...
    $("#iperfexSendChanPass").hide(10,function(){
        $("#iperfexImgSavingPass").css("display","inline");
    });
    //mostramos la imagend e loading
    arrAction["menu"]          = '_iperfexutils';
    arrAction["action"]        = "changePasswordIperfex";
    ...
    request("index.php",arrAction,false,
        function(arrData,statusResponse,error)
        {
            if(statusResponse == "false"){
                alert(error);
                $("#iperfexImgSavingPass").hide(10,function(){
                    $("#iperfexSendChanPass").css("display","inline");
                });
            }else{
                alert(arrData);
                hideModalPopUP();
...
function addBookmark(){
    var arrAction = new Array();
    arrAction["menu"] = "_iperfexutils";
    arrAction["id_menu"] = getCurrentIperfexModule();
        ...
        function(arrData,statusResponse,error)
        {
            ...
            if(statusResponse == "false"){
                ...
                if(/bookmarkon.png/.test(imgBookmark)) {
                    ...
                    $("#togglebookmark").attr('src',"web/themes/iperfexneo/images/bookmark.png");
                } else {
                    var labeli = $("#toolTip_removeBookmark").val();
                    $("#togglebookmark").attr('title', labeli);
                    $("#togglebookmark").attr('src',"web/themes/iperfexneo/images/bookmarkon.png");
                }
                alert(error);
            }else{
...
function loadDetails(){
    var order = "menu=_iperfexutils&action=versionRPM&rawmode=yes";
    ...
    $( "#search_module_iperfex" )
        ...
        .autocomplete({
            ...
            source: function(request, response){
                ...
                $.ajax({
                    url: 'index.php?menu=_iperfexutils&action=search_module&rawmode=yes',
                    dataType: "json",
                    data: {
            ...
            open: function() { // parche que resuelve el bug del panel de busqueda de modulo en PBX
                ...
                if(top_var == "0px" & left_var == "0px"){
                    var searchPosition = $('#search_module_iperfex').position();
            ...
            close: function() {
                $('#neo-cmenu-showbox-search').one('click', function(e) {
                    //$( "#search_module_iperfex" ).autocomplete( "close" );
                    $( "#search_module_iperfex" ).val("");
                    e.stopPropagation();
                });
...
//Recoger el valor del módulo activo a partir de iperfex_framework_module_id
function getCurrentIperfexModule()
{
	return $('#iperfex_framework_module_id').val();
}

function getWebCommon(){
    return $('#iperfex_framework_webCommon').val();
}