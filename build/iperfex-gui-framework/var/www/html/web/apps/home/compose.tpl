...
<div id='iperfex-compose-email'>
    <div id='compose-headers-div' class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <table id='compose-headers-table'>
                <tr id='compose-to'>
                    <td></td>
                    <td class='iperfex_compose_htd'><label for='compose_to'>{$TO}:</label><textarea name='compose_to' class='iperfex_compose_textarea'>{if $USERNAME}{$USERNAME}{/if}</textarea></td>
                </tr>
                <tr id='compose-cc' style='display:none'>
                    <td class='iperfex-del-compose-header'> <span class='glyphicon glyphicon-minus-sign'></span></td>
                    <td class='iperfex_compose_htd'><label for='compose_cc'>{$CC}:</label><textarea name='compose_cc' class='iperfex_compose_textarea' ></textarea></td>
                </tr>
                <tr id='compose-bcc' style='display:none'>
                    <td class='iperfex-del-compose-header'> <span class='glyphicon glyphicon-minus-sign'></span></td>
                    <td class='iperfex_compose_htd'><label for='compose_cco'>{$BCC}:</label><textarea name='compose_bcc' class='iperfex_compose_textarea'></textarea></td>
                </tr>
                <tr id='compose-reply_to' style='display:none'>
                    <td class='iperfex-del-compose-header'> <span class='glyphicon glyphicon-minus-sign'></span></td>
                    <td class='iperfex_compose_htd'><label for='compose_replay_to'>{$REPLYTO}:</label><textarea name='compose_replay_to' class='iperfex_compose_textarea'></textarea></td>
                </tr>
                <tr id='compose-extra-headers'>
                    <td></td>
                    <td class='iperfex_compose_htd'>
                    <a href="#cc" id='iperfex_link_cc' onclick='showComposeHeader("cc")' class='iperfex_compose_header_link'>{$CC}</a>
                    <span> | </span>
                    <a href="#bcc" id='iperfex_link_bcc' onclick='showComposeHeader("bcc")' class='iperfex_compose_header_link'>{$BCC}</a>
                    <span> | </span>
                    <a href="#reply_to" id='iperfex_link_reply_to' onclick='showComposeHeader("reply_to")' class='iperfex_compose_header_link'>{$REPLYTO}</a>
                    </td>
                </tr> 
                <tr id='compose-subject' style='margin:5px 0'>
                    <td></td>
                    <td class='iperfex_compose_htd'><label for='compose_to'>{$SUBJECT}:</label><input name='compose-subject' style='width:100%; border:1px solid #999;'></input><td>
                </tr>
            </table>
        </div>
    </div>
    <div id='iperfex-compose-msg-attach' style='margin-bottom: 2px;'>
        <div class='iperfex-compose-msg-attachitem' id='login_loading_attach' style='display:none'>
            <img src='{$WEBCOMMON}images/loading.gif' /> {$TEXT_UPLOADING}
        </div>
    </div>
    <div id='iperfex-compose-msg'>
    </div>
</div>
<input type='hidden' name='iperfex_language' value="{$USER_LANG}">
...
<input type='hidden' name='iperfex_txtuploading' value='{$TEXT_UPLOADING}'>
<input type='hidden' name='iperfex_txtattach' value='{$TEXT_attach}'>
<input type='hidden' name='iperfex_txtsend' value='{$TEXT_send}'>
{literal}
    <style type="text/css">
        .mce-widget button {
            height: 28px;
        }
        .iperfex_compose_textarea{
            resize: none;
            width: 100%;
            border: 1px solid #999;
        }
        .iperfex-del-compose-header{
            vertical-align:top;
        }
        .iperfex_compose_htd{
            width: 100%;
        }
        #iperfex-compose-msg-attach{
        ...
        .iperfex-compose-msg-attachitem{
        ...
        .iperfex-compose-msg-attachitem img{
            width:18px;
            height:16px;
            padding-left: 2px;
            border:0;
    ...
    <script type="text/javascript">
        ...
        function showComposeHeader(header_field){
            ...
            $("#iperfex_link_"+header_field).hide();
            if(header_field=='reply_to'){
                if($("#iperfex_link_bcc").is(':hidden')==false){
                    $("#iperfex_link_bcc").next('span').hide();
                }else if($("#iperfex_link_cc").is(':hidden')==false && $("#iperfex_link_bcc").is(':hidden')==true){
                    $("#iperfex_link_cc").next('span').hide();
                }
            }else{
                $("#iperfex_link_"+header_field).next('span').hide();
            }
        }

        function emailAttachFile(){
            var txtUploading=$("input[name='iperfex_txtuploading']").val();
            var divAttach=null;
            $('#attachFileButton').liteUploader(
            {
                ...
                {   //mostrar que se esta cargando el archivo adjunto
                    var div_itemattach="<div class='iperfex-compose-msg-attachitem' id='login_loading_attach'>";
                    div_itemattach +="<img src='web/_common/images/loading.gif'/>"+txtUploading;
                    div_itemattach +="</div>";
                    $("#iperfex-compose-msg-attach").append(div_itemattach);
                    divAttach=$("#iperfex-compose-msg-attach").children(':last');
                },
                success: function (response)
                {
                    ...
                    if(response.error !== ''){
                        ...
                        attachInfo +="<a href='#' id='"+response.message['idAttach']+"'  onclick='emailDetachFile(\""+response.message['idAttach']+"\")'><img src='iperfex/web/themes/iperfexneo/images/bookmarks_equis.png'></a>";
        ...
        function richTextInit(){
            user_language=$("input[name='iperfex_language']").val();
            tinymce.init({
                selector: "#iperfex-compose-msg",
                plugins: [
                ...
                resize: true,
                menubar : false,
                //auto_focus: "iperfex-compose-msg",
                /**
        ...
        **/
        function composeEmail(fromaction){
            ...
            var bodyMsg = tinyMCE.get('iperfex-compose-msg').getContent();        
            if(fromaction!='popup'){
                showIperfexUFStatusBar("Sending...");
            }
            var arrAction = new Array();
            arrAction["menu"]="home";
